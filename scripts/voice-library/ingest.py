...
def import_local(voices_dir, src_dir, cache_dir):
    ...
    # Read the manifest BEFORE the loop: Phase 3 auditions are generated by a separate pass
    # and live only in the manifest, so a rebuilt entry must carry them forward or every
    # re-import silently deletes 120 clips' worth of references. Same defect class as the
    # `performanceClips: []` hardcode found in session 18 — a well-formed manifest with the
    # work quietly gone. Keyed by id, so a voice that is re-imported keeps its own auditions
