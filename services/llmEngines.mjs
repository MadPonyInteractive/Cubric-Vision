/**
 * LLM backends — Ollama, ComfyUI, DeepInfra. Two consumers, ONE implementation:
 * the Stage 1 recipe harness (`scripts/recipe-test.mjs`, which imports
 * `scripts/recipe-engines.mjs`, now a re-export of this file) and the app's own
 * enhance route (`routes/llm.js`, MPI-677 step 1a).
 *
 * MOVED HERE FROM `scripts/` FOR ONE REASON: `scripts/` is excluded from the
 * portable build (`scripts/build-portable.mjs:135`). A route importing it from
 * there would work in dev and be absent in the shipped app. Nothing in the file
 * was edited in the move — see the verbatim note below, which is now doing two
 * jobs at once.
 *
