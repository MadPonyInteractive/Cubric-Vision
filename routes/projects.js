 ...
 *   comfyViewUrl  {string}  — http://127.0.0.1:48188/view?filename=...
 *   itemId        {string}  — UUID generated by the client before calling this route
 *   operation     {string}  — command key, e.g. 't2i', 'upscale'
 *   filePrefix    {string}  — optional filename prefix override; defaults to `operation`
...
router.post('/project/save-generation', async (req, res) => {
    try {
        ...
        // Sanitise the filename prefix (max 24 chars). The op key is the default; a
        // command may override it so the card reads as the name the user picked —
        // a Flow's key is internal (`flowChatterBox`) while the Library shows its
        // title ("Text to Speech"). MPI-660. `operation` itself is untouched: it is
        // what the sidecar stores and what Reuse resolves against.
        const prefix = String(filePrefix || operation).replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 24);
 ...
 *   x             {number}  — crop origin x in image-space pixels (integer, may be negative)
 *   y             {number}  — crop origin y in image-space pixels (integer, may be negative)
 *   w             {number}  — crop width in image-space pixels (integer)
 *   h             {number}  — crop height in image-space pixels (integer)
 *   fill          {string}  — optional '#rrggbb' for pixels outside the source (MPI-383)
 *   outW          {number}  — optional resample width  (RESOLUTION crop family only)
 *   outH          {number}  — optional resample height
 *
 * The rect may extend past any edge; the overhang is padded with `fill` before
 * the extract (Sharp's .extract alone throws out of bounds). See
...
router.post('/project/crop-media', async (req, res) => {
    try {
        const { folderPath, itemId, sourceFilePath, x, y, w, h, fill, outW, outH } = req.body;
        ...
        // Sharp crop — pads first when the rect leaves the image (MPI-383)
        let cropExtended;
        try {
            ({ cropExtended } = require('../services/imageCrop'));
        } catch (_) {
            return res.status(500).json({ success: false, error: 'Sharp is not installed on the server.' });
        }

        const written = await cropExtended(inputPath, filePath, { x, y, w, h, fill, outW, outH });
...
/**
 * POST /project/composite-media
 *
 * Blends two project images through a mask (MPI-362). Neither source is
 * modified — the blend is saved as a new history entry, so it is undone by
 * deleting that entry. Mask WHITE = take the overlay, black = keep the base;
 * the caller picks the direction (Add / Subtract) by which image it sends as
 * the base. Overlay + mask are stretched to the base's pixel size.
 *
 * Body:
 *   folderPath   {string} — absolute project folder path
 *   itemId       {string} — UUID generated by the client before calling
 *   basePath     {string} — source kept OUTSIDE the mask (abs path or /project-file URL)
 ...
 */
router.post('/project/composite-media', async (req, res) => {
    try {
        const { folderPath, itemId, basePath, overlayPath, maskDataUrl, feather, fillHoles } = req.body;

        if (!folderPath)  return res.status(400).json({ success: false, error: 'folderPath required' });
        if (!basePath)    return res.status(400).json({ success: false, error: 'basePath required' });
        if (!overlayPath) return res.status(400).json({ success: false, error: 'overlayPath required' });
        if (!maskDataUrl) return res.status(400).json({ success: false, error: 'maskDataUrl required' });

        const id = itemId || uuidv4();
        const baseAbs    = decodeProjectFilePath(basePath)    || basePath;
        const overlayAbs = decodeProjectFilePath(overlayPath) || overlayPath;

        for (const p of [baseAbs, overlayAbs]) {
            if (!(await fs.pathExists(p))) {
                return res.status(400).json({ success: false, error: 'Source file not found: ' + p });
            }
        }
        ...
        const filename = await nextSequence(folderPath, mediaDir, 'composite', ext);
        const filePath = path.join(mediaDir, filename);

        const maskBuffer = Buffer.from(String(maskDataUrl).replace(/^data:[^;]+;base64,/, ''), 'base64');
        const { compositeThroughMask } = require('../services/imageComposite');
        const { width, height } = await compositeThroughMask({
            basePath:    baseAbs,
            overlayPath: overlayAbs,
            maskBuffer,
            outPath:     filePath,
            feather:     Number.isFinite(Number(feather)) ? Number(feather) : undefined,
            fillHoles,
        });
...
/**
 * POST /project/apply-paint
 *
 * Flattens the canvas paint layer onto its entry as a NEW history entry (MPI-375).
 * The source is untouched, so this is undone by deleting the new entry.
 *
 * A SIBLING of composite-media, not a mode of it: that route blends two project
 * FILES through a mask, and paint has neither — one file plus an RGBA layer that
 * carries its own alpha. Only the surrounding machinery is shared (nextSequence,
 * the .meta sidecar, the thumbnail, the response shape).
 *
 * The layer arrives as a data URL because it exists only in the renderer; the
 * SOURCE never round-trips as base64 (the MPI-362 rule), it is read off disk.
 *
 * Body:
 *   folderPath      {string} — absolute project folder path
 *   itemId          {string} — UUID generated by the client before calling
 *   sourceFilePath  {string} — image painted onto (abs path or /project-file URL)
...
router.post('/project/apply-paint', async (req, res) => {
    try {
        ...
        if (!(await fs.pathExists(baseAbs))) {
            return res.status(400).json({ success: false, error: 'Source file not found: ' + baseAbs });
        }
        ...
        const { compositeOverlay } = require('../services/imageComposite');
        const { width, height } = await compositeOverlay({
            basePath: baseAbs,
            overlayBuffer,
            outPath: filePath,
