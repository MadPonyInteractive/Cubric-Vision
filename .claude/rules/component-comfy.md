...
**Auto-mask has THREE detector branches (MPI-361 → rebuilt on SAM3 in MPI-380, text added MPI-384).** Full graph contract: `docs/masking-sam3.md`. `img_auto_mask.json` injects `Input_Image`, `sams`, `Input_Box`, `Input_Selected_Masks_Input`, plus:
...
**Every branch converts its MASK back to SEGS with `MaskToSEGS(combined=True)` and rejoins the existing `ImpactSEGSPicker` → `ImpactSEGSToMaskList` chain** — that is what preserves the shipped 1.2.0 one-mask-per-pick contract (never `SegsToCombinedMask`; the JS guards on `length === picks.size`). On the text branch that same node is also what SPLITS a `[N_obj,H,W]` batch into one SEG per object, because Impact's `mask_to_segs` loops `mask.shape[0]` — which is why `individual_masks` must be TRUE there.
