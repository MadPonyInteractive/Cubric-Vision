...
| `previewStage` | `MpiButton` (size: sm, toggleable, icon: frameForward) | `"Input_Video_Latent"` (`MpiStageLatents`, the `is_preview` WIDGET) | run payload `previewOnly: boolean`; `commandExecutor._buildParams` injects `Video_Latent.is_preview: <boolean>` **only for multi-stage ops** so non-multi-stage workflows do not receive a stray field. **`payload.historyMode === true` overrides the toggle to force it false** regardless of persisted value — video-history workspace must never produce preview cards. **The standalone `Preview_Only` boolean node/param is GONE (MPI-473)** — do not re-add it. | `t2v_ms`, `i2v_ms` |
...
| `removeBackground` | `MpiToolOptionsRemoveBg` (tool panel) | `"Input_Bg_Use_Color"` (MpiIfElse gate), `"Input_Bg_Color"` (`EmptyImage.color`) | `{ Input_Bg_Use_Color: boolean }`, plus `{ Input_Bg_Color: int 0xRRGGBB }` only in Color mode. Transparent → `JoinImageWithAlpha` (RGBA); Color → `ImageCompositeMasked` over `EmptyImage`. BiRefNet native node outputs a MASK; `InvertMask` feeds the transparent branch, the un-inverted mask feeds the color composite. `color` field added to `_inject` targets (MPI-260). Persisted under `toolSettings.removeBackground`. | `removeBackground` |
...
| `denoise` | `MpiProgressBar` (interactive, wheel, handle) | `"Denoise"` (`MpiFloat.inputs.float`) | `{ Denoise: 0..1 }` (float, step 0.01) | model-tied `upscale` (default 0.20), `detail` (default 0.30), **`pid` (default 0.0 → drives PiD degrade_sigma via the `Input_Denoise` tier-2 alias — MPI-182)** — defaults via `commands[op].defaults.denoise`; persisted under `operations[opName].denoise` so each op has independent state |
| `pidVariant` | `MpiRadioGroup` (sm, 4 cols: Flux\|SD3\|Qwen\|SDXL) | `"Input_Type"` (`MpiAnySwitch.inputs.select`, **1-indexed**) | `{ Input_Type: 1\|2\|3\|4 }` (1=flux, 2=sd3, 3=qwen, 4=sdxl) — PiD VAE-path selector. Needed `'select'` added to the `_inject` targets (MPI-182). | `pid` |
| `pidResolution` | `MpiRadioGroup` (sm, 3 cols: 1K\|2K\|4K) | `"Input_Resolution"` (`MpiAnySwitch.inputs.select`, **1-indexed**) | `{ Input_Resolution: 1\|2\|3 }` (1=1K, 2=2K, 3=4K native passthrough) — PiD output size. | `pid` |
...
**Auto-mask has THREE detector branches (MPI-361 → rebuilt on SAM3 in MPI-380, text added MPI-384).** Full graph contract: `docs/masking-sam3.md`. `img_auto_mask.json` injects `Input_Image`, `sams`, `Input_Box`, plus:

- **points** — `Input_Points_Mode` (bool) + `Input_Points_Positive` / `Input_Points_Negative`, plain JSON pixel coords on **`MpiText`** nodes.
- **text** — `Input_Text_Mode` (bool) + the dotted `Input_Text_Prompt.text`, written straight onto `CLIPTextEncode`'s own widget.

The branches sit behind **chained** `MpiIfElse` gates (`Input_Text_Mode` → `Input_Points_Mode` → `Input_Box`) whose inputs are **lazy**, so only the selected one executes.

- **`MpiText`, NOT `MpiString`** (MPI-380 shipped this after being bitten): `MpiString` is in `comfyController`'s `PATH_MEDIA_CLASSES`, so any param aimed at one is treated as MEDIA and pushed through `_resolveMediaPath` / `_uploadRemoteMedia` — the remote leg then tries to upload a file named after the JSON. `CLIPTextEncode` is not in that set, which is why the text prompt needs no relay node at all.
- **Nothing in the graph gates an empty run any more.** The old points loader self-gated via `ExecutionBlocker`; string nodes cannot. `MpiCanvasViewer` refuses both empty-points and empty-text runs with a toast — load-bearing, not a nicety.
- **DELETED, do not reintroduce:** `Input_Points_Mask` (the `data:` dot image), the dotted `Input_Points.threshold`, and the Scope dial. SAM3's point path ignores threshold entirely. `tests/auto-mask-inject-titles.test.cjs` asserts all three are gone.

**Every branch converts its MASK back to SEGS with `MaskToSEGS(combined=True)` and rejoins the shared `ImpactSEGSToMaskList` fan-out** — that is what preserves the shipped 1.2.0 one-mask-per-pick contract (never `SegsToCombinedMask`). **The `ImpactSEGSPicker` that used to sit in front of it is DELETED (MPI-421)**: it trimmed the masks to the chips selected at dispatch, so a chip toggle had to re-run the graph. `Output_image` now emits EVERY object's mask, index-aligned with the `Output_Detected` thumbs because both halves walk the same SEGS list — feed them from different nodes and picks paint the wrong objects while the counts still match. On the text branch that same node is also what SPLITS a `[N_obj,H,W]` batch into one SEG per object, because Impact's `mask_to_segs` loops `mask.shape[0]` — which is why `individual_masks` must be TRUE there.
...
| `batch`        | `MpiOptionSelector` (variant: 'number') | `'Batch_Size'` (`MpiInt.inputs.int`) | `1` | `{ Batch_Size: 1\|2\|3\|4 }`. **scope: `shared`** — `project.shared[mediaType].batch`. |
...
| `previewStage` | `MpiButton` (toggleable) | `'Input_Video_Latent'` (`MpiStageLatents`, the `is_preview` WIDGET) | `false` | does NOT contribute to `injectionParams` (`getInjectionParams()` returns `{}`); instead `el.getRunPayload()` reads the toggle and sets payload `previewOnly: boolean`. `commandExecutor._buildParams` injects `Video_Latent.is_preview: <boolean>` **only for multi-stage ops** — single-stage workflows never receive the field, so a stale toggle from a prior `_ms` op cannot leak into them. The standalone `Preview_Only` param was deleted in MPI-473. **scope: `shared`** — `project.shared[mediaType].previewStage`. |
...
| `inpaint`         | Inpaint            | image     | 1              | —             | true         | **no**         | (none)              | active — replaces `change`+`remove` (both retired from commandRegistry, `deprecated: true` in operationRegistry). Prompt-OPTIONAL on purpose: with a prompt it replaces the masked area, empty it erases and fills. First wired by FLUX.2 Klein 4B; other flux editors reuse it. |
...
| `removeBackground`| Remove Background  | image     | 1              | —             | —            | no             | (none)              | universal   |
