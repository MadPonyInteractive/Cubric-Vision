...
**Auto-mask has TWO detector branches (MPI-361).** `img_auto_mask.json` injects `Input_Image`, `sams`, `Input_Box`, `Input_Selected_Masks_Input` and — added by MPI-361 — `Input_Points_Mode` (bool), `Input_Points_Mask` (a `data:` URL of white dots on BLACK, read through `ImageToMask(channel=red)`) and the dotted key `Input_Points.threshold`. The branches sit behind an `MpiIfElse` titled `Input_Points_Mode` whose inputs are **lazy**, so the unselected branch never executes; in detector mode the points loader self-gates on the empty path via `ExecutionBlocker`. `Input_Points_Mask` needs no bespoke plumbing on either engine — its node class is in `comfyController`'s `PATH_MEDIA_CLASSES`, so the `data:` URL is staged to a real file and uploaded to the Pod automatically. **The points branch converts its MASK back to SEGS with `MaskToSEGS(combined=True)` and rejoins the existing `ImpactSEGSPicker` → `ImpactSEGSToMaskList` chain** — that is what preserves the shipped 1.2.0 one-mask-per-pick contract (never `SegsToCombinedMask`; the JS guards on `length === picks.size`).
