...
**Frontend entry point:** `js/services/downloadService.js` — a singleton mirror of the backend store snapshot. Always use `downloadService.start/cancel/uninstall()`, never raw `fetch` directly. On `download:snapshot` it REPLACES `state.downloadJobs` wholesale (version-gated). `_start()` shows an optimistic client-only `pending` job that reverts to Install after 10s with a `ui:warning` toast if no backend ack lands.

**`start()` is the MPI-451 LICENCE GUARD; `_start()` is the job creation described above.** A model carrying a descriptor in `js/data/modelConstants/licences.js` must accept a terms dialog before it installs. A model with NO descriptor must keep reaching `_start` in the **same tick** — `MpiModelManager._install()` relies on the synchronous `download:started` to patch its tile and flip the detail footer to Cancel, so never make `start()` wholesale `async`. Full contract, including the receipt shape and three traps that are invisible in source: `docs/download-manager.md` § The licence gate.

**The SOT is `routes/install/installStore.js` (MPI-276).** One store, legal-transition table (illegal moves rejected + logged), monotonic `version`. **No `refCount` — DELETED; never reintroduce it, never gate on `refCount === 0`.** "Is a dep in-flight" = `store.activeModelsForDep(depId)` / `_inFlightDepIds`. `routes/install/reconciler.js` drives the store from disk/volume truth (settles wedged jobs, fails orphans, prunes, broadcasts the snapshot) on SSE connect and a 15s poll. **Two traps — they are how MPI-396 hid:** the poll **self-idles** (`store.hasActiveJobs()` gate), so once every job is terminal it never runs again; and the post-uninstall `reconcileOnce()` is on the **LOCAL leg only** (the remote leg returns `res.json()` first). Neither can settle an uninstall, and `pruneTerminal` structurally cannot express confirmed-*UN*installed (its DONE branch drops on `confirmedInstalled.has(id) || age >= DONE_TTL_MS`, and an uninstalled model is in neither) — so **uninstall settles the store itself**: `store.dropModel(modelId)` + `broadcastSnapshot()` at BOTH legs. A guard test pins that at exactly two call sites; do not "fix" a stale post-uninstall job in the tile renderer instead (`docs/model-library.md`).
