...
**Workflow JSON: agents MAY author it, and the runtime files stay GENERATED (Fabio, 2026-08-22).** The old blanket ban — *"never edit workflow JSON, EVER; files under `comfy_workflows/` are owned by the user"* — is lifted. It rested on a recovery worry that no longer holds (everything here is committed) and on an incident that agents did not cause: MPI-272's API-format file in `raw/` was a user **Export (API)** mis-click, and the real agent-side hazard — a bare `workflow-to-api.mjs` run — was fixed in code by `assertNotInRaw()` (`f918c907`). What replaces it:

- **`comfy_workflows/raw/` is authorable.** It holds **LiteGraph, never API JSON** — that format law is the part that was always load-bearing. Prefer round-tripping a real graph (the ComfyUI frontend auto-lays-out an API import, and `app.graph.serialize()` reads the LiteGraph back out) over synthesising one; `docs/workflow-authoring/bench-editing.md` forbids hand-writing LiteGraph **nodes**, because widget-to-input ordering is implicit and shifts silently.
- **`comfy_workflows/*.json` is BUILD OUTPUT, not a source file.** Produce it with the converter against the **app engine** (`COMFY_URL=http://127.0.0.1:48188`), never the bench, and gate it with `validate-injection-rules.mjs`. Hand-patching a converted API file is still wrong — not because of ownership, but because the next convert silently overwrites it. Fix the raw graph and re-convert.
- **Diff the round trip against the graph that actually RAN, value by value**, before shipping. Node and class counts match on a graph the editor has quietly quantised — see `bench-editing.md` § The editor QUANTISES float widgets, where `step: 0.01` rounded a signed-off `12/255` threshold to `0.05` with no error anywhere.
- **Re-titling is still the user's call when the app-touched titles are wrong** — see the paragraph above. Authoring a graph does not mean inventing an injection contract.
...
**Every sampler's `noise_seed` must LINK to the `Input_Seed` node — never a baked literal (MPI-257).** Two independent bugs stacked here and neither would have fixed it alone: Boogu-Edit's exported template had one sampler chain with `noise_seed` hardcoded to `22`, so that tier's graph was byte-identical every run and ComfyUI genuinely cached it — while `_hasSeedNode` was matching title `=== 'seed'`, a title NO workflow in the fleet uses, leaving the dedupe guard dead fleet-wide and harmless only because a fresh random seed normally defeats the cache. Result: a false "no settings changed" toast and no re-gen. When authoring or reviewing a workflow, grep `noise_seed` in the runtime JSON and assert every one is `["<id>",0]` pointing at the `Input_Seed` `MpiInt`. Any code keying on the seed node matches `input_seed` case-insensitively, never `seed`. A "same output every re-gen" report on a new model → check BOTH halves.
...
| `"Input_Use_Transition"` | `inputs.boolean` | **(LTX-2.3 — MPI-127.)** `MpiBoolean`. The i2v motion/lipsync enabler — the transition LoRA it switches on is what makes lipsync work at all. Forced `true` by the `audioMode` control whenever audio is present (either mode). |
...
| ~~`"Input_Preview_Only"`~~ / ~~`"Input_Is_Continue"`~~ | — | **DELETED with their nodes (MPI-473).** Were `MpiBoolean` gates required on multi-stage base workflows. **Zero shipped graphs carry either node** — the preview/continue gate is now the `Input_Video_Latent` widget pair on the row below. `_buildParams` no longer emits them, and the `comfyController` defensive strip that warned `Preview_Only requested but workflow has no matching node` on EVERY multi-stage run is gone with them. Do NOT re-add either as an injected param. |
| `"Input_Video_Latent"` | `inputs.load_path` + the `is_continue` / `is_preview` widgets | **Required on every multi-stage workflow.** Now an **`MpiStageLatents`** node, not `LoadLatent` (MPI-466 finished the migration; WAN and H3 went first). ONE node replaces the old save/load/gate cluster, so there is no `_stage2` twin file. Its flags are WIDGETS, addressed as `Input_Video_Latent.is_continue` / `.is_preview` — `_buildParams` emits them for every multi-stage op. Stage 2 reads `load_path`, which `POST /comfy/stage-preview-latent` fills with the per-preview `<previewUuid>.latent`. **Nothing is staged for stage 1 any more**: `WORKFLOW_INPUT_DEFAULTS` and `POST /comfy/prepare-workflow-inputs` were deleted with the last `LoadLatent`. |
...
| `"Output_Preview"` | read-only | **Required on multi-stage video workflows** — a `SaveVideo` carrying the preview clip on `Input_Video_Latent.is_preview=true` runs. `commandExecutor.js` maps the preview-only capture title to `'output_preview'` (finals match `'output_video'`). No `Output_Audio` on preview (throwaway clip; audio only on the final `Output_Video`). (Was bare `Preview` pre-MPI-252 — the base `'preview'` string survives only as a defensive fallback.) |
...
- **Bypass mechanic (MPI-223):** the per-slot bypass toggle injects `strength_model=0, strength_clip=0` (it does NOT remove the node or reload the model — that would OOM; the same reason an A/B of two LoRAs flips `strength` 0↔0.5 rather than bypassing, with the seed locked first). A slot whose file is absent is **silently skipped**, not blocked.
