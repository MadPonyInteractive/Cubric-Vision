...
**Multi-stage is per-MODEL, not per-op (MPI-127).** The `_ms` op keys (`t2v_ms`/`i2v_ms`) are SHARED across WAN and LTX. Whether a model actually exposes the preview/stage-2 flow is gated by `model.capabilities.multiStage`: WAN = `true` (shows the `previewStage` toggle, runs the two-file flow below); LTX this release = `false` (toggle hidden in `MpiPromptBox`, ships single-stage — the stage-2 `LTX_*_stage2.json` files are GENERATED and ready, but preview→continue is deferred to MPI-128 because it needs DUAL-latent staging, video + audio). So "an `_ms` op = multi-stage" is only true when the active model declares `multiStage`. The stage-1-only ship path does NOT use the stage-2 file.
...
- A `SaveLatent` node titled `"SaveLatent"` that emits the stage-1 latent on preview runs. (LTX saves TWO — video + `Input_Audio_Latent`; dual stage-2 staging is MPI-128.)
