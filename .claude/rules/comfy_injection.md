...
Injection matches node title EXACTLY and **silently drops any param whose title matches no node** — no error, no log, no toast. THE mechanism behind a family of invisible bugs. **MPI-242 hit it twice:** a batch node titled `Input_Batch` never matched (the injector emits `Input_Batch_Size` — a PURE prefix, never abbreviated), so Batch N rendered 1 image in Krea2 *and* shipped Chroma; and `Input_Is_i2i` existed only in source COMMENTS, so Krea2 i2i ran as t2i for four sessions. `CommandDef.injectParams` (constant per-op params keyed by title, merged in `_buildParams` before user params) makes branch-selecting booleans declarative but does NOT remove the skip. **The diagnostic the injector refuses to give is `tests/inject-params-titles.test.cjs`** — it asserts every `injectParams` title exists in every workflow its op runs; run it when wiring a new op onto a shared graph. Also trace a gated control to its real consumer before mounting it (Krea2 `denoise` reaches the sampler only through the `Input_Is_i2i` gate — live on i2i, inert on t2i/pose). Same family as the `Output_Image` typo trap (title-map row) and the MPI-229/198 path-heal separator mismatches.
