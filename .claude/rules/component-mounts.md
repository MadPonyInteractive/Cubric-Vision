...
- `MpiButton` (cogwheel — parameters popup trigger, MPI-356)   props: `{ icon:'settings', variant:'ghost', size:'sm', info:'Generation parameters' }`   slot: `#settings-cog-slot` — FIRST control after the textarea (bar order: neg · prompt · **cog** · enhance · model · engine · run). Grid columns are all `auto` except the prompt's `1fr`, so DOM order alone sets the position.
