...
- Landing project rows (one per project)   slot: `#projectGrid` children — rebuilt on every `loadProjectGrid()`. **NOT a component:** since the Stage redesign these are hand-built `.mpi-landing__pl-row` divs from `_buildProjectRow()` in `js/shell/projectUI.js`, so a row change goes in `_buildProjectRow` + `styles/shell/landing.css` (see `docs/workspaces.md` § Landing). **There is no project-card component to reach for.** `MpiProjectCard` was deleted in MPI-739 — the component gallery turned out to be its last mount anywhere, and the `projectUI.js` import that outlived it was already dead. Do not re-create one from an old draft. Per-row stats (asset count + bytes-on-disk) come from `fetchStats()` in `js/services/projectStatsService.js`; in-flight fetches are aborted via the module-local `_statsBatchAC` AbortController when the grid rebuilds, so late responses don't write into rows that no longer exist.
- **`#landingActions` slot:** plain `<a>` text links (`Models · Flows · Settings · Remote · Hotkeys · About`) — NOT `MpiButton`s. `Models` / `Flows` emit `models:open` / `flows:open`; the other four dispatch `Events.emit('slide-over:open', { title, component })` where `component` is one of `MpiSettings | MpiRemote | MpiHotkeys | MpiAbout` (imported as content blueprints, not mounted directly). Hero version label `Cubric Studio · v${APP_VERSION}` mounts at `#heroVersion`.
...
> - **Toggle plates — `MpiCheckbox` props `{ checked, variant: 'switch' }`, and NO `label`.** Every Settings toggle goes through the local `_mountSwitchPlate(slotId, checked, onChange)`, which mounts the switch into the plate's ctrl slot and keeps the plate's `.mpi-settings__plate--on` heat outline in sync with it; the row's text is the plate's own markup, so a `label` prop here would render a second one. Slots: `#mpiSettingsAutoStartSlot`, `#mpiSettingsRecycleBinSlot`, `#mpiSettingsNotifyGenerationSlot`, `#mpiSettingsNotifyDownloadsSlot`, `#mpiSettingsNotifyConnectionSlot`, `#mpiSettingsToastSoundSlot`, `#mpiSettingsFloatLatentSlot`. The variant has a gallery card (MPI-739).
...
>
> **`MpiRunpodSettings` (MPI-177) and `MpiLlmSettings` are NOT in Settings — both MOVED to `MpiRemote` (MPI-728).** Settings is about this machine; Remote is about somebody else's. `MpiRemote.setup()` mounts each ONCE (not per `onOpen`) — props `{}`, slots `#mpiRemoteLlmMount` and `#mpiRemoteRunpodMount`. `MpiRemote.el.onOpen()` forwards to both children's `el.onOpen()` (RunPod's runs `_initRunpodSection`); `MpiRemote.el.destroy()` destroys both. RunPod's internal mounts (key/DC/volume/GPU/connect controls) still target `#mpiSettingsRunpod*` slots inside its own template — the ids survived both moves verbatim.
> `MpiLlmSettings` rebuilds on every `onOpen`: `MpiInput` (`type:'password'`) at `#mpiSettingsLlmKeySlot`, `MpiButton` Save / Clear at `#mpiSettingsLlmKeySaveSlot` / `#mpiSettingsLlmKeyClearSlot`, and `MpiDropdown` (`extraClasses:'mpi-dropdown--stacked'`) at `#mpiSettingsLlmEnhanceBackendSlot`, `#mpiSettingsLlmEnhanceModelSlot` (its group hides for ComfyUI) and `#mpiSettingsLlmDescribeBackendSlot`.
...
- `MpiLoraRack` (Compound, MPI-724)   props: `{ modelId }`   slot: `#settings-lora-slot` — FIRST child of `.mpi-prompt-box__settings`, above the control grid: the current model's FILLED LoRA slots, each a name label + strength input(s) + bypass. Mounted ONCE, after `cogBtn` (a row-count change re-anchors the popup and `positionPopup()` measures against the cog); its `resized` listener is attached AFTER `.mount()` because mount renders synchronously and would call `positionPopup` before `cogBtn` exists. Refreshed from the tail of `_refreshOpSlot()` via `el.setModel(model?.id ?? null)` — ONE call site, the convergence point every `model` reassignment funnels through. `openPopup` does NOT refresh it: the rack's own `settings:model:update` listener is live whether the popup is shown or not, and a read there lands inside projectService's 300ms debounce (it showed an EMPTY rack for LoRAs just set). Holds an Events subscription, so `el.destroy()` is called from the box's `destroy` — it lives inside the portaled `popupNode`, not in `el`. Renders nothing at all when the model has no LoRAs. It can never add, remove or re-point a LoRA; that stays on the model card (MPI-356)
...
- `MpiFlowResultDock` (the floating result window, MPI-727)   props: none   slot: `#flow-stage` — **mounted beside the slides, never inside one, and that IS the fix.** The run slide is rebuilt on every navigation, so a result living only there exists only while the user stands on the last step: Fabio played a song, stepped back to read his lyrics, and the sound stopped with the slide. Built at the TOP of `setup` (not beside `_syncDock`) because the seeded-result HEAD probe can clear the window synchronously during mount. Outlives the slide like `MpiModelSettings` — destroyed in `el.destroy()`, not `_teardownSlide()`.
  ...
  - 🔴 **`_syncDock()` must stay SYNCHRONOUS inside `_renderSlide`, after `slidesEl.appendChild(slide)` and BEFORE the `requestAnimationFrame` on the next line.** A media element removed from the document is paused *"once a stable state is reached"* — after the current task, not during it — so the shared `<audio>` re-appended inside that one pass never stops, and the same element moved one tick later does. Only AUDIO is shared (`_sharedAudioEl`, keyed by url): the spec mutes and loops video and thumbnails images, so `_dockNode` builds fresh elements for those and `MpiVideoViewer` / `MpiVideoControlBar` / `MpiCompareView` are untouched by this. `tests/desktop/flow-result-follows-steps.spec.js` pins it by node identity.
