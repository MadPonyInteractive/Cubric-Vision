...
COMPOSITE API (MPI-373 — a SCRATCH hole plus an underlay; full contract in `docs/composite.md`):
         `setCompositeUnderlay(url)` — point the tool at image 2. Resolves **false** when it will not load; the panel rolls its slot back on that and SAYS so.
         `setCompositeHoleFromMask()` — Mask Comp: take the cut from the selected entry's own mask. Reads `MaskManager.getURL()` with **NO arguments** (white-on-transparent). The `('black','white')` overload is OPAQUE, and the hole is consumed by **alpha** here but by **luminance** on the server — the opaque one cuts the whole frame on screen and only the white part on disk.
         `hasCompositeHole()` / `getCompositeURL()` — the gate and the mask Apply sends. `isEmpty()` SCANS alpha rather than trusting a flag, which would go stale on undo.
         `clearComposite()` — wipe the cut as ONE undo entry; the slot image stays. `resetComposite()` drops BOTH, and is what `discardPreview()` calls.
         `setOnCompositeChange(fn)` — ONE callback slot, not a subscription: the panel is rebuilt on every rail switch and `instance.on()` hands back no unsubscribe. **Every path that changes the cut must announce through it** — `loadImage`, `clearComposite`, `setCompositeHoleFromDataURL`, `_applyUndo`, and the end of a cut stroke. It shipped broken once: Apply reloads the entry it just created, `loadImage()` wiped the cut, nothing told the panel, and the next press returned silently at its own null guard.
NOTE:    The composite hole is its OWN scratch layer (`holeCanvas`), never persisted, and the BRUSH MEANING IS INVERTED — eraser grows the hole, brush shrinks it. Painting it into the mask layers was rejected: the mask persists per entry, so a composite would eat a mask brushed for an inpaint. Reading that mask is fine; writing into it is not.
...
#### MpiToolOptionsComposite (Organism — js/components/Organisms/MpiToolOptionsComposite/)
PROPS:   `{ viewer, mode: 'maskComp'|'paintComp', clipboard }` — `clipboard` is `{ hasImage(), getImage() }`, ACCESSORS not values, because the panel mounts once and the buffer changes under it.
EMITS:   `composite-apply` `{ overlayUrl, maskDataUrl }` — **its own event, deliberately not `apply`**, so `_handleApply`'s "no canvas tool reaches the generation branches" guard stays hard. The Block routes it to the EXISTING `_runComposite()` → `POST /project/composite-media`; it is a server-side blend, not a generation, and passes no `fillHoles` (MPI-437 made that opt-in).
GLOBAL EMITS: (none of its own — the shared `MpiMaskStrip` writes `toolKey: 'composite'`, a bucket nothing reads back)
LISTENS: (none — it claims `viewer.el.setOnCompositeChange()` on mount and clears it in destroy)
NOTE:    ONE component under BOTH `maskComp` and `paintComp` (`_COMPOSITE_TOOLS`), a `MOUNTS` table for the differences, unknown `mode` **THROWS**. **THE ONLY GROUP THAT DROPS THE PROMPTBOX** — in `_isCanvasTool`, absent from `_modeKeepsPromptBox`; collapsing those two predicates hands the box straight back and a test guards it. Apply is gated on BOTH halves (an image underneath AND a non-empty cut) and renders **disabled, not inert**. Full subsystem: `docs/composite.md`.
NOTE:    **The hint line is the panel's ERROR surface.** An image the canvas rejected and an entry with no mask are both otherwise silent — the slot renders its own thumbnail regardless, and a greyed Apply gives no reason. Derived from flags, not written per caller: the slot seed and the entry-mask read resolve asynchronously, so a `_say(msg)` API would let whichever landed second erase the other's reason.

#### MpiMediaSlot (Compound — js/components/Compounds/MpiMediaSlot/)
