...
- **Mascot** — canonical 5-file set in `assets/mascot/`: `logo.png`, `idle.png`, `greet.png`, `waiting.png`, `happy.png`. Landing empty uses `greet`; `MpiToast` maps variants to keys (`success`→`happy`, `warning`→`greet`, `info`/`danger`→`idle`). **The running-generation mascot lives on TWO independent surfaces — change BOTH:** (1) gallery card, `MpiGalleryGrid._setMascotState` (MPI-265) — `idle` = big centered, flipping `idle↔greet` on a self-rearming 4–8s timer; `cooking` (first preview/latent lands) = small bottom-right showing `waiting.png`, flip stopped; (2) history-viewer peek, `MpiGroupHistoryBlock` `mascot-peek` / `_mascotShow`/`_mascotHide` — one `<img>` at 66×66 bottom-right showing `waiting.png` while generating, hidden on complete, floated by the `mascot-float` keyframe. **The on-complete `happy` flash is DEAD on both surfaces** — the finished result paints over it, so it is never seen: the history block just calls `_mascotHide(0)` and the gallery card rebuilds. `happy` survives only on the success toast. Don't re-add a happy flash to either surface.
...
* Z-index contract: the body-portal layer is **`11000`** — `MpiDropdown`'s list, `MpiTreePicker`, `MpiStylePicker`, `MpiColorPicker`'s popup. It sits ABOVE a full-page overlay (`overlayManager` `BASE_Z` 10000, +10 per depth) and BELOW toasts (20000), so a select or a swatch inside an overlay renders correctly. Plain `.mpi-popup` is `9999`, which is **below an overlay**: anything portaled to `body` that can open inside one must be on 11000, and a component carrying both classes needs the double-class form (`.mpi-popup.mpi-x__popup`) or load order decides the tie. MPI-606: the colour picker sat on 9999 and painted underneath the Flow overlay.
* Viewport clamp: after placing a portaled popup by trigger coordinates, measure `getBoundingClientRect()` in `requestAnimationFrame` and clamp BOTH axes. `left` — popups anchored with `translateX(-50%)` clip off-screen near viewport edges otherwise. `top` — flip above the trigger when the popup will not fit below, and clamp when it fits neither way; a tall popup (the colour picker is ~354px) opened from a trigger in the lower half of the window loses its bottom row entirely, and horizontal-only clamping hides that until someone opens it low (MPI-606).
...
5. **HIDDEN ≠ DESTROYED (MPI-345)** — an overlay component whose close only calls `overlay.el.hide()` keeps every binding its `setup` made. `Hotkeys.bind` allows MANY handlers per id and `_handle` fires ALL of them, so a closed-but-alive component's hotkey runs alongside the visible one's: a closed Flow's `generation.run` queued a phantom generation on the next Ctrl+Enter. Either **destroy on close** (`MpiBaseFlow` — the shell emits/handles `close`, and every open remounts anyway) or **bind on open / unbind on close** (`MpiCompareOverlay`). Never bind a GLOBAL handler in `setup` for a component that outlives its own visibility.
6. **A MOUNT-TIME READ GOES STALE WHEN THE ENTRY CHANGES UNDER THE PANEL (MPI-373)** — `mountOptions()` rebuilds a tool panel on every RAIL switch but **not** when the user selects a different history entry. A panel that reads per-entry state once in `setup` (the entry's mask, its dimensions, whether it has paint) is therefore correct exactly until the first entry change, and then silently wrong — with the tool still open and nothing to indicate it. This shipped as a Composite Apply button that went permanently dead on the next entry and only came back via a rail switch. Either re-read on the canvas' own load path (`MpiCanvasViewer.loadEntry()`, **after** `_restoreLayers()` — earlier reads the entry the user just left), or do not cache per-entry state in a panel at all. Note the usual escape is closed here: the factory's `instance.on()` returns no unsubscribe, so a short-lived panel subscribing to the long-lived viewer leaks one listener per mount — which is why the composite family uses a single callback SLOT (`setOnCompositeChange`) instead.

7. **HIDDEN ≠ RESIZED — a `ResizeObserver` MUST ignore a 0×0 measurement (MPI-597)** — an element inside a `display:none` subtree has no box, and `getBoundingClientRect()` returns zeros for it. That is not a resize, it is *not rendered*, and every overlay in this app produces it: `MpiOverlay`'s Stash Pattern moves the whole workspace under a `display:none` div, so opening the Model Library fires the observer of every canvas underneath with an empty rect. A handler that treats those zeros as a real size corrupts state that has to survive the round trip — `MpiCanvas.resize()` fed `ViewManager.handleResize(w, h, 0, 0)` on the way out, whose own `oldW > 0` guard then skipped the matching pass on the way back, leaving a panned view displaced by `-(w/2, h/2)` and clipped into the top-left corner for the rest of the session. **Bail first: `if (!rect.width || !rect.height) return;`** — and keep the last REAL size, because that is what the restore pass compares against. Caching the zeros is the same bug in a different place.
