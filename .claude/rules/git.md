...
3. Commit the INDEX: bare `git commit -n`, **no pathspec at all**. `-n` bypasses the lint-staged stash/reapply — run eslint yourself first; you are opting out of the hook, not the check.
   - **Precondition: the index must hold ONLY your files.** Check `git diff --cached --name-status` first — no pathspec means this commits a PEER's staged work too (adds *and* deletes). The co-owned case forces no-pathspec while a peer's staged files forbid it; the two halves of this rule collide head-on. When they do, do NOT improvise a commit: finish every other close-out step and re-check, since a peer mid-commit usually lands inside your session and clears the index by itself. If it still has not cleared, stage into a scratch index instead — `GIT_INDEX_FILE=<tmp> git read-tree HEAD`, then `git apply --cached` + `git commit -n` under the same env var — which leaves the real index untouched. (Collision seen 2026-07-29; the wait resolved it. **Fallback EXERCISED 2026-08-14, MPI-564** — the peer's staged foley files never cleared, `GIT_INDEX_FILE=<tmp> git read-tree HEAD` + `git add <my paths>` + `git commit -n` under the same env var committed 9 files with the real index untouched.)
   - **The scratch index leaves the REAL index stale, and the next peer commit weaponises it (MPI-564).** Your commit moves HEAD; the real index still holds the OLD blobs, so `git diff --cached` now reports your just-committed files as staged **deletions** (`D` for anything you added) and staged reverts (`M` back to the old content). Nothing warns you — the working tree is correct and `git log` is correct. A peer's next bare `git commit -n` then applies that: your new card folder deleted, your component reverted, in a commit that looks like theirs. **Immediately after the scratch-index commit, run `git reset HEAD -- <your paths>`** (index-only, working tree untouched, not the banned `--hard`), then re-check `git diff --cached --name-status` shows ONLY the peer's entries again.
4. Confirm the sibling's files are still `M` (modified, uncommitted) afterwards.

**RE-CHECK `git log` BEFORE RE-RUNNING A FILTER — once the peer LANDS, the same recipe commits a REVERT of their work (MPI-567, 2026-08-24).** The filter is defined by what it CUTS, and it has no idea whether that content is uncommitted or now in `HEAD`. A session that filtered `flowsRegistry.js` twice over MPI-607's uncommitted Voice Changer block re-ran it an hour later; MPI-607 had committed in between, so the third staging produced a blob that *deleted* their FlowDef, their op, their workflow entry and their test — a clean, lint-passing, `node --check`-passing revert of two commits, staged and one keystroke from landing. Nothing about it looks wrong: the tree is correct, the tests pass, and `git status` shows the file as modified exactly as before.
- **The tell is the SIGN of the leak.** A guard that greps the staged diff for the peer's markers fires either way, but on the first run the hits are `+` lines (their work leaking in) and after they land they are `-` lines (their work being cut out). Assert on `^-`, not just on the marker.
- **The fix is to stop filtering, not to fix the filter.** Once their half is in `HEAD`, the working tree IS the correct union — plain `git add <your paths>` is right and the co-owned problem has dissolved.
- Cheapest guard: `git log --oneline -5` before every re-run of a staging script, and make the script itself refuse when a cut block's markers already exist in `HEAD`.
