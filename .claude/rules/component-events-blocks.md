...
API:     `el.getRunPayload()` returns the current live run payload. Loop re-fire reads it via `getNextGeneration` callback so prompt/model/control changes apply to the next iteration.
         ...
         Op-change contract (MPI-337): a LOST requirement (mask cleared, maskless History card, media removed) NEVER switches the op — it stays selected + renders disabled; Run toasts the missing input (`generationService` media/mask guards). The op changes ONLY on media-add force-UP (`_emitMediaChange`: a text-only op + staged media → the smallest-fitting media op) and Reuse. No force-DOWN.
...
LISTENS: `workspace:set-operation` `{ operation: string }` — syncs PromptBox operation
         `ui:open-model-picker` — opens the Block-owned `MpiModelPicker` with the workspace-filtered installed list (video history stays i2v-only), applies the pick (MPI-356)
...
NOTE:    Reads `state.currentProject`; writes `state.currentProject`
         ...
         **Active tool:** block-local `_options` (current MpiToolOptions* instance). NOT in global `state`. `mountOptions(mode)` is the mediator — destroys previous instance, mounts new one into `#right-top-slot`. `prompt` mode toggles `--prompt-active` CSS class (shows PromptBox, hides slot) and is the ONLY mode that swaps the viewer surface (`swapToPreview`/`swapToCanvas`). No channel bus for tool events.
         **PromptBox is ALSO live on every mask tool (MPI-372)** — `_isMaskTool(mode)` force-mounts + shows it while KEEPING the canvas, so `--prompt-active` stays off and `#right-top-slot` still hosts the tool panel. Do NOT add a surface swap for a mask tool: `swapToPreview()` destroys `MpiCanvas` to free GPU texture backing, so swapping mid-mask kills the brush. Every other rail tool still hides the box.
         **`workspace:set-operation`** (op-strip only emitter; the radial no longer sends it) validates the op against live context and sets it — it must NOT force `setMode('prompt')`. That radial-era force swapped the canvas out from under an active mask.
         **Tool re-arm:** `_syncViewerToolMode()` re-enters the canvas mode from the RAIL (source of truth) and is shared by BOTH reload paths — `historyList 'entry-selected'` and `_reloadViewerWithEntry` (generation:complete / gallery:item-updated). Never call `exitMode()` before `loadEntry`: `loadEntry` captures and restores the mode itself, so pre-clearing leaves an armed rail tool over a dead canvas.
         **Image groups:** mask tools → one per masking method (MPI-371): `maskDetect` → MpiToolOptionsMaskDetect, `maskPoints` → MpiToolOptionsMaskPoints. No apply button; additive composite; auto-detect composites onto existing manual paint. Both mount the shared MpiMaskDetectRow + MpiMaskStrip; B/E hotkeys live in MpiMaskStrip and only when it is mounted `brush: true` (not on Points). Rail modes are `maskDetect`/`maskPoints` but the viewer knows only `'mask'` — `_viewerModeFor()` bridges them at the two re-arm sites. A tool swap must never clear `manualCanvas`/`subtractCanvas`.
