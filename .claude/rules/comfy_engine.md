...
- **`installRequirements: true/false` is the POD BAKE/VOLUME SPLIT, not a claim about the node's requirements.** `true` nodes BAKE into the Pod image (pip cost paid at build); `false` nodes install onto the volume at connect (no rebuild to bump them). Orthogonal to universality. See § "Node commit-drift ladder" below.
  ...
  - **`pipPins` / `installRequirementsCommand` are DEAD DATA — do not reach for either.** Both engines converged on the curated file (2026-08-04), so nothing executes them any more: the Pod wrapper runs no pip at all, and the `install_command` / `pip_pins` passthrough in `routes/remoteModels.js` is deleted. They survive only as fields in `nodesDeps.js`, pinned by `tests/node-drift.test.cjs` (every baked node must declare pins) and `tests/controlnet-aux-torch-guard.test.cjs`. A version you need on either engine goes in `dev_configs/python_deps.in`, never here.
