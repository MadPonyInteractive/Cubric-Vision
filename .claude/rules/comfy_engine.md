...
- **`installRequirements: true/false` is the POD BAKE/VOLUME SPLIT, not a claim about the node's requirements.** `true` nodes BAKE into the Pod image (pip cost paid at build); `false` nodes install onto the volume at connect (no rebuild to bump them). Orthogonal to universality. See § "Node commit-drift ladder" below.
  ...
  - **`pipPins` is DELETED and `installRequirementsCommand` is DEAD DATA — do not reach for either.** Both engines converged on the curated file (2026-08-04), so nothing executes them any more: the Pod wrapper runs no pip at all, and the `install_command` / `pip_pins` passthrough in `routes/remoteModels.js` is deleted. `pipPins` itself went with it (MPI-630, 2026-08-28) along with the `tests/node-drift.test.cjs` invariant that demanded one per baked node — that assertion was propagating dead data onto every new pack (MelodramaBox went red until pins were invented for it). `installRequirementsCommand` survives as a field on two entries, unexecuted. A version you need on either engine goes in `dev_configs/python_deps.in`, never here.
