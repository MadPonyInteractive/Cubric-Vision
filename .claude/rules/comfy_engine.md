...
**Adding a custom-node dep — nodes are UNIVERSAL by TYPE (MPI-222):** every `type: 'custom_nodes'` dep is in the universal set — there is no longer a model-specific node class (the old `installOnEngine` flag + `getInstalledModelNodeDeps()` are DELETED). `getUniversalWorkflowDepIds()` selects `type==='custom_nodes' || engineAsset===true`, so every node installs with the engine and every `engineAsset` weight (upscalers, yolo/sam, RIFE) comes with it. The ONE knob on a node entry is **`installRequirements: true/false`** — orthogonal to universality — set it whenever the node ships a real `requirements.txt` (it also gates Pod BAKE vs VOLUME, see below). A node's pinned commit lives in `dev_configs/node_lock.json` (`source: git-commit|git-tag|registry`); bumping it there triggers the drift ladder (marker `.mpi_node_commit`) on both engines — no other edit needed. **One exception, and it is mandatory (MPI-413): if the node has `installRequirements: true`, adding OR bumping it also requires regenerating the curated pip set** — `node scripts/compile-node-deps.mjs --check`, add what it reports to `dev_configs/python_deps.in`, `node scripts/compile-node-deps.mjs`, commit both files. The local engine runs NO node's `requirements.txt`; it installs `dev_configs/python_deps.txt` in one `--no-deps` pass, so a node missing from that file ships with missing dependencies. Never hand-edit `python_deps.txt`, and never add `torch`/`torchvision`/`torchaudio`/`triton`/`nvidia-*`/`cuda-*` to it (engine-owned) or a second opencv distribution (they share the `cv2` namespace). `pipPins` (MPI-630) and `installRequirementsCommand` (MPI-646) are both DELETED — neither field exists on any entry, and neither engine has run a pack requirements resolve since the Pod converged — the curated file is the only place a version can be set.
...
**NEVER place a weight in `G:\ComfyUi\ComfyUI\models\` (the standalone bench's own models folder).** The one local model store is **`G:\CubricModels\<type>\`** — both the bench (hand-written `extra_model_paths.yaml`) and the app engine (generated by `routes/yamlHelper.js`) point at it, and it mirrors the R2 layout. A weight in the bench's own `models/` is visible to the bench alone: invisible to the app engine, absent from R2, and it makes a dep read "installed" while every other consumer is missing it. If a node needs a folder type the bench yaml lacks, create `G:\CubricModels\<TYPE>\` and add `<key>: <TYPE>/` under `cubric_models:`. The key is the ComfyUI **folder-type id**, not the folder name — read it off the node's own `add_model_folder_path()` call, since a node may register a lowercase type for an uppercase dir. `extra_config.py` registers any key you write, so a typo fails silently as an empty dropdown. Never hand-edit the ENGINE yaml (regenerated on every `/comfy/set-path`); a gap there is a `coreExtras` code fix.
...
- **A WEIGHT that must ship with the engine** (upscalers, yolo/sam detectors, RIFE) → `engineAsset: true`. It pulls the file into the universal install set on BOTH engines. **On remote it is image-resident only when it also carries `bakedOnPod: true` (in the Pod Dockerfile `dl` block) or `targetPath` (baked inside a node folder); otherwise it VOLUME-INSTALLS on first connect** via `shell.js _installRemoteEngineAssets` (MPI-380). **Unflagged is the safe default** — it costs a volume download, while a wrong `bakedOnPod` makes the weight unreachable on remote and the failure is a 503 mid-generation on a billed Pod. Only add `bakedOnPod` in the same breath as the Dockerfile edit; `tests/remote-engine-assets.test.cjs` guards the two in lockstep.
- **`installRequirements: true/false` is the POD BAKE/VOLUME SPLIT, not a claim about the node's requirements.** `true` nodes BAKE into the Pod image (pip cost paid at build); `false` nodes install onto the volume at connect (no rebuild to bump them). Orthogonal to universality. See § "Node commit-drift ladder" below.
  - **It does NOT mean "this node has no requirements.txt" (MPI-472).** Read literally as "set `true` when the node ships a real `requirements.txt`" — which is what this line said until 2026-08-07 — it reads as a promise that a `false` node declares nothing. `ComfyUI-VideoHelperSuite` is `false` and ships two lines, one of which (`imageio-ffmpeg`) is a hard runtime requirement of **our own** `MpiSaveVideo`. Since MPI-413 nothing runs a node's `requirements.txt` on either engine, so `dev_configs/python_deps.in` is the ONLY installer and every locked node's declared requirements must land in it regardless of this flag. `compile-node-deps.mjs --check` used to filter on it and printed `OK` while the requirement was missing; it now reads every node in the lock. Cost: every video op on every model died at the last node, after the full sample, on the first engine that fully reinstalled.
  - **MANDATORY PASS when you add or bump ANY locked node (MPI-413):** the LOCAL engine no longer runs any node's `requirements.txt`. It installs one curated file, `dev_configs/python_deps.txt`, in a single `--no-deps` pass. Run `node scripts/compile-node-deps.mjs --check`, add whatever it reports to `dev_configs/python_deps.in`, regenerate with `node scripts/compile-node-deps.mjs`, and commit BOTH files (`.in` is hand-curated, `.txt` is generated — never hand-edit it). Skip this and the node ships with missing dependencies and fails to import on the user's machine. `tests/curated-python-deps.test.cjs` guards the two invariants: no engine-owned torch stack (`torch`/`torchvision`/`torchaudio`/`triton`/`nvidia-*`/`cuda-*` — a naive compile pulls them in as real transitives of diffusers/ultralytics/kornia) and exactly one opencv distribution (they share the `cv2` namespace). Full step: `docs/playbooks/add-model/02-dependencies-r2.md`.
  - **`pipPins` and `installRequirementsCommand` are both DELETED — neither field exists, do not reach for either.** Both engines converged on the curated file (2026-08-04), so nothing executed them any more: the Pod wrapper runs no pip at all, and the `install_command` / `pip_pins` passthrough in `routes/remoteModels.js` is deleted. `pipPins` went first (MPI-630, 2026-08-28) along with the `tests/node-drift.test.cjs` invariant that demanded one per baked node — that assertion was propagating dead data onto every new pack (MelodramaBox went red until pins were invented for it). `installRequirementsCommand` went the same way (MPI-646, 2026-08-28), from `nodesDeps.js` **and** from `dev_configs/node_lock.json`, and took `tests/controlnet-aux-torch-guard.test.cjs` points 1-3 with it — the same shape of assertion, guarding a pip run that no longer happens. `tests/curated-python-deps.test.cjs` now guards both directions: the LOCAL install path must not regain the step, and the field must not come back as data. A version you need on either engine goes in `dev_configs/python_deps.in`, never here.
...
**Engine upgrade/wipe MUST preserve the custom models root:** any op that wipes the engine folder destroys `extra_model_paths.yaml`. Capture the root FIRST with `getCustomRoot()` (shared.js — reads `base_path:`) and pass it to `_runEngineDownload(preservedRoot)`, which re-writes it at step 6. `hasCustomRoot` is a boolean flag, NOT the value — never rely on it for preservation. Otherwise the user's custom folder (e.g. `D:\CubricModels`) silently resets to the default `mpi_models` → 0 models → no prompt box. (MPI-118.) **`/engine/upgrade` no longer wipes by default (MPI-457):** it moves the engine IN PLACE (`git fetch --tags` + checkout the `node_lock.json` pinned sha + pip for only the requirement lines that moved + restamp `.mpi_engine_version`), because the portable ships ComfyUI as a real git checkout. `_fullEngineReinstall()` (the wipe, preservation and all) is still there and is reached by a DETECTED signal — a dead tree, a deprecated node carrying our `.mpi_node_commit` marker, a moved engine-owned package (`torch`/`triton`/`nvidia-*`/`cuda-*`), or any in-place failure. Signal table + traps → `docs/playbooks/bump-engine/02-local-upgrade.md`.
