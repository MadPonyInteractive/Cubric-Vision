...
| **"Auto_Mask"** | `inputs.boolean` (Boolean) | Toggle for auto masking vs manual. |
...
| **"Ready"** | `inputs.boolean` (Boolean) | Signal to run the detailing process (true) vs just detection (false). |
...
| **"Output_Mask"** | N/A (Read-only) | **REQUIRED for Visual Feedback**. Identifying the node that outputs the combined mask image (Black=Mask, White=Background). |
