...
- [ ] Gates 7-9 — **DEFERRED to a later session, on the user's call**: the H3
      ...
      Resume with: `node scripts/smoke-workflows.mjs --plan` (the pod-lock drift warning
      should now be GONE — that is the proof gate 6 held), then gate 7 asserts the Pod
      reports 0.30.0, then the matrix.
