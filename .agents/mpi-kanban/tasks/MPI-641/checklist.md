...
- [ ] NOT DONE: no screenshot. `app:isolated` exits 0 in silence while a peer holds uncommitted edits in `main.js` + `scripts/launch-instance.mjs`, and the overlay is zero-size on Landing so `locator.screenshot` refuses it. See validation.md.
