...
## 2026-08-26 — Curation pass and the full import (session 18)

### What the corpus actually is, and why "register spread across R1–R5" is not achievable

All 228 CC0 `voice-donations/` voices were measured (`ingest.py --measure-only`, ~139 MB
cached outside the repo). **227 usable**: `boom` downloads fine but `pyin` finds zero voiced
frames in it — genuinely unusable as a reference, not a pipeline fault.

| R1 90–130 | R2 130–190 | R3 190–260 | R4 260–340 | R5 340+ | below 90 Hz |
|---|---|---|---|---|---|
| **131** | **62** | **17** | **1** | **1** | **15** |

The corpus is overwhelmingly low-male. **R4 is one voice (`Aon`, 263.2 Hz) and R5 is one
(`Glenn`, 365.9 Hz).** No amount of curation produces a child or cartoon register from this
source; that is a sourcing problem, not a selection problem. `Aon` also scores `snr_proxy`
7.7 dB, so a gate anywhere at 8 dB or above would have **zeroed R4 outright** — which is how
the gates below ended up where they are rather than at a rounder number.

Full f0 range 60.7–365.9 Hz; deciles 60.7 / 95.8 / 104.5 / 109.4 / 116.2 / 122.1 / 126.4 /
135.5 / 142.7 / 169.7.

### The gates were set FROM the distribution, not before it

This card has already paid for one threshold picked in advance (the absolute 3.5 st span gate
calibrated on R1's neutral, which wrongly failed R3). So all 227 were measured first and the
gates set against the observed tails.

- **`voiced_frac >= 0.35`** — cuts the genuinely broken tail (`WhisperInEar` 0.078,
  `Selfie` 0.067). Under 35% of a 10 s clip is under 3.5 s of actual voice.
- **`snr_proxy >= 6.0 dB`** — the same tail from the other side (`WhisperInEar` −10.1 dB).

**Three signals were measured and deliberately NOT used as gates**, which is the more useful
half of this result:

- **`peak_dbfs` discriminates nothing here.** The corpus is already peak-normalised: max
  −1.0 dBFS, min −6.3, and **zero** voices clip. Still measured, because it would catch a
  future corpus that is not normalised — but it separates nothing in this one.
- **`duration_s` discriminates nothing.** Only 4 voices fall under 10 s and the shortest is
  7.08 s, ample for a VC reference.
- **`span_st` is a RANK PENALTY, never an exclusion.** A wide p10–p90 does track worse clips
  (mean `voiced_frac` 0.455 vs 0.549 above 16 st) but it is primarily a `pyin` octave-error or
  creak tell — and this card already recorded a 21.4 st span on audio a listener passed. Gating
  on it would have dropped `3973` (24.5 st) and `spanish-limaperu` (17.8 st), two of only
  seventeen R3 voices. It breaks ties where there is a choice; it never excludes from a scarce
  register.

### The selection: 60 voices, correcting the corpus bias rather than reproducing it

R3/R4/R5 taken **whole** (all 19 that pass). R1 and R2 quota'd down and spread across their
band by contiguous f0 quartile, best quality-rank within each quartile.

| | R1 | R2 | R3 | R4 | R5 |
|---|---|---|---|---|---|
| available (post-gate) | 114 | 60 | 17 | 1 | 1 |
| **shipped** | **26** | **15** | **17** | **1** | **1** |
| share of corpus → share of bundle | 58% → 43% | 27% → 25% | 7% → 28% | — | — |

R1 picks span 97.5–129.4 Hz, R2 130.1–169.7 Hz. `voices/curated.txt` records every selection
with its reason and every rejection with the gate it failed — it is the audit trail, and it is
regenerated by `--curate`, never hand-edited.

### Two defects found and root-fixed, neither reported by anything

1. 🔴 **The import would have DELETED the twelve authored Phase 2 clips.** `run()` hardcoded
   `"performanceClips": []` on every write. The next import of any kind would have wiped the
   whole grid and left a perfectly well-formed manifest behind — no error, no warning. It now
   reads whatever is on disk and carries it through; **check 9** exists specifically to fail if
   that ever regresses.
2. 🔴 **My own f0-quartile spreading was a stride, not a quartile.** `by_f0[i::4]` hands each
   group a sample of the *whole* range, which is the opposite of spreading — and it made the
   "f0-quartile 1/4" note in `curated.txt` a claim the code never made. The first selection
   skewed high as a result (22 of 26 R1 picks above 110 Hz). Fixed to contiguous quarters, and
   the note now carries each quartile's actual Hz bounds so the claim is checkable.

### Verification

`scripts/voice-library/check_manifest.mjs` (new, 12 checks) runs against the **real loader**,
not a re-implementation, so pipeline/app drift fails here instead of silently emptying a filter
in the picker. Calibrated on the old 10-voice manifest first (12/12, and check 8 independently
reproduced the handoff's `R1:6 R2:3 R3:1 R4:0 R5:0`) before anything changed.

- **12/12 contract checks** at 60 voices — loader accepts, all 18 brief § 3 fields present,
  `accent` null on all 60, every `median_f0` in its own band, all 60 opus present and non-empty,
  `kind:'both'` in both filters, register buckets partition exactly, **the 12-clip grid intact**,
  no orphan opus, bundle 3.05 MB.
- Check 11 **caught the 8 orphans** the curation left behind (the old sample minus `3973`/`4189`)
  and they were pruned. It is not wired into `npm test` — run it after any import.
- **737/737** node suite green.
- **Idempotent**: a second `--ids-file` run produced a byte-identical `manifest.json`
  (sha256 `4cb406b0…de106`).
- Bundle **3.05 MB** at 60 voices, inside D1's 5 MB estimate.

### Open — both are Fabio's calls, neither blocks Phase 3

- 🔴 **15 voices sit below R1's 90 Hz floor** (60.7–89.4 Hz; `Deepak` 60.7, `Butter` 74.7,
  `7020` 85.3 at a healthy 0.692 voiced / 13.6 dB). The band table in `brief.md` § 2 has no home
  for them, so they were **excluded rather than silently filed under R1** — the old code would
  have assigned them R1 with a warning, which would fail contract check 4. Extend R1's floor,
  add an R0, or leave them out? Deep-bass character voices are exactly the kind a user reaches
  for, so this is not a cosmetic gap.
- 🔴 **R4 and R5 have one voice each.** Child and cartoon character voices effectively do not
  exist in this corpus. The performance grid already covers R1+R3 only, so nothing is blocked
  today — but the picker will show a register with a single entry.
- ⚪ **`kind: "both"` on all 60 may be CORRECT, not a defect.** The handoff flagged the inert
  kind filter as a curation problem. It is not: a real human clip genuinely serves both the
  direct-TTS and the VC-target route, so the filter is inert because the corpus is *uniform*,
  not because the import is wrong. No `kind`, `gender` or `age` labels were invented from
  measurements — same rule that keeps `accent` null.


---

## 2026-08-26 — THE CORPUS IS REJECTED. Source changes to VoiceDesign (session 18)

### Fabio's verdict on the 60 curated voices: none of them ship

*"I think 99% of this voice library is garbage... none of it is usable."* His reasons, in his
order: **unintelligible accents** (the donation pool is heavily Indian- and Spanish-accented —
*"I can't even understand what they're saying"*), **poor microphone quality**, and a
**cartoon that is a bad voice actor impersonating one**. On top of the structural gap already
measured: R4 = 1 voice, R5 = 1 voice, out of 228.

He had already bounced off this corpus independently — *"I came across one, and everything was
[unintelligible]. That's why I never mentioned it."*

### Why the curation could not have caught it — the failure is mine and it is structural

The gates were `voiced_frac` and `snr_proxy_db`. **Those measure SIGNAL, not SPEECH.** They
cannot see accent, intelligibility, microphone character or acting. I wrote precisely that
limitation into the code — *"None of these is a perceptual verdict"* — and then shipped a
60-voice selection on them without listening to a single clip.

**The sharpest part: the attribute that decided usability was the one field the design forbids
inferring.** `accent` is null on every entry by rule (brief rule 1; MPI-607 closed
accent-inference as NEGATIVE), so accent could not enter the ranking even in principle — and
accent is what killed it. It also means the picker's accent filter would have been dead on
arrival regardless of source.

This card already carried the lesson one layer down: *"the CAMPPlus cosine agreed with the
pipeline at every step while disagreeing with the listener."* I repeated it. **The standing
correction: a perceptual product needs a perceptual gate BEFORE volume — one voice per
category to the ear, then the rest.** That gate is now built into the workflow, at Fabio's
suggestion, not mine.

### The new source: Qwen3-TTS VoiceDesign — a promotion, not a pivot

`brief.md` § Sourcing already listed it as the long game. It fixes every rejection reason:
studio-clean by construction, and its **uncontrollable American prior** — closed NEGATIVE on
MPI-607 when the goal was *choosing* an accent — is exactly the one consistent intelligible
house accent needed here. Same defect inverted by the goal, for the second time on this card.
Apache-2.0, offline authoring, never an app dependency.
...
### The taxonomy — APPROVED by Fabio 2026-08-26. 12 categories x 5 = 60

| # | Category | Target register | | # | Category | Target register |
|---|---|---|---|---|---|---|
| 1 | Deep male | R1 | | 7 | Standard female | R3 |
| 2 | Standard male | R1 | | 8 | Young female | R3 |
| 3 | Elderly male | R1 | | 9 | Elderly female | R2 |
| 4 | Young male / teen | R2 | | 10 | Child | R4 |
| 5 | Narrator / trailer | R1 | | 11 | Cartoon / critter | R5 |
| 6 | Mature female | R2 | | 12 | **Villain / menacing** | R1 |

`register` stays the ENGINEERING key (which shared performance grid a voice draws on), never
a product label. **A library voice carries no emotion** — it is one neutral sample; emotion
arrives at runtime from the shared register x emotion grid.

**Category 12 was renamed from `creature_monster`** on Fabio's ear: *"Creature monster sounds
more like a villain. Sounds like a menacing voice."* A good result wearing the wrong label, so
the label moved. It is also the ONE mixed-gender category (*"the variants can have a male and
female"*) — see `VILLAIN_VARIANTS` in `research/library_personas.py`.

🔴 **TRUE CREATURE/MONSTER AND ROBOT ARE BOTH POST-FX, NOT TTS.** `brief.md` already settles
the robot case (*"post-FX (vocoder / ring-mod / formant shift) on any voice — a later card"*)
and a monster is the same shape: non-human timbre comes from processing, not from a model
whose entire prior is human voices. Neither needs a library slot — both apply to ANY deep
voice. Fabio agreed and noted he never asked for a monster.

### Library v1 — 12 voices, one per category. Fabio judged 5

`research/library_personas.py`, 12/12 generated, 0 failed, level-matched to -20 dBFS.

| category | measured f0 | Fabio's ear |
|---|---|---|
| elderly_male | 107.2 Hz | **"not elderly, he sounds 25"** + dead air at both ends |
| mature_female | 172.1 Hz | **"sounds 30"** (asked for forties) |
| elderly_female | 209.5 Hz | **"sounds about 50"** (asked for seventies) |
| child | 509.8 Hz | **"hurts my ears... almost like a critter"** |
| creature_monster | 92.8 Hz | villain, not a creature → renamed |

**Seven categories are still UNJUDGED**: `deep_male`, `standard_male`, `young_male`,
`narrator_trailer`, `standard_female`, `young_female`, `cartoon_critter`.

### Three measured findings behind those calls

1. 🔴 **The child's harshness is f0, NOT an EQ boost.** Fabio guessed "boosting 500 Hz or
   1 kHz a lot" — his ear localised it exactly right (26.7% of energy in 300–700 Hz plus
   40.8% in 700 Hz–1.5 kHz = 67.5% in that span) but the cause is different: the fundamental
   sits at **509.8 Hz**, about an octave above a real eight-year-old, so there is **0.0% of
   energy below 300 Hz** to give it body (vs 48.3% for `standard_female_1`). It also measured
   HIGHER than the cartoon critter (462 Hz) — the child was more critter than the critter.

2. 🔴 **My silence measurement was WRONG and Fabio's ear was right.** He reported ~3 s of
   silence at the head and ~5 s at the tail of `elderly_male_1`; `librosa.effects.trim(top_db=35)`
   reported only 1.11 s / 0.29 s. **A single transient at 1.11 s anchored the trim.** The RMS
   envelope shows the truth: 0–4 s mean −40.4 dB, 4–12 s speech, 16–20 s mean −40.6 dB. Fixed
   with a **sustained-energy trim** (5 consecutive frames must clear the threshold) in
   `prep_matrix_audition.py`; it must move into the shipping pipeline.

3. 🔴 **Register/pitch steering is loose — 7 of 12 v1 voices missed their target band**, and
   two misses are product problems rather than cosmetics: `mature_female` (172.1) and
   `standard_female` (174.1) are **2 Hz apart**, so nothing but delivery separates two
   categories; and the female ladder is scrambled (`young_female` 328.7 sits above both).

### The age/pitch matrix — 4 categories x 3 wordings x 2 seeds, v1 kept as CONTROL

`research/age_pitch_matrix.py`, 24/24 generated. The design point: **n=1 cannot tell "the
prompt is not conditioning the output" from "this seed was unlucky"** — Phase 2 burned exactly
that trap when the flat-cell hypothesis was falsified by its own control.

**The two axes behaved DIFFERENTLY, and that is the result:**

- 🟢 **PITCH responds to wording. Decisive on the child**: between-wording spread **139.4 Hz**
  against a largest within-wording spread of 44.3 Hz. v1 454–466 Hz → v2_lower 327–371 →
  v3 340–382, and the low-end body came back with it (`<300 Hz` energy 0.4% → 16.6%).
  **Best candidate `child__v2_lower__s0` at 326.8 Hz, in R4, 16.6% low energy.**
- 🔴 **AGE does NOT respond to wording.** Jitter stayed **2.29–4.42%** across every wording in
  every category, including v2/v3 which named tremor, creak, breathiness, unsteady pitch and
  weak breath support explicitly. On `elderly_male` the within-wording spread (65.0 Hz) is
  effectively the between-wording spread (74.0 Hz) — **seed variance, not the prompt.** v2's
  jitter was if anything *lower* than v1's, the wrong direction for an elderly voice.

**Do not over-read the jitter number.** It is a proxy, and trusting a metric over the ear is
the exact mistake that produced the rejected corpus. The matrix page exists so Fabio judges
whether v2/v3 *sound* older; the measurement only says the acoustic markers I can see did not
move.

### State of the repo

The 60 rejected kyutai voices are **still in `voices/`**, deliberately. Deleting them now would
