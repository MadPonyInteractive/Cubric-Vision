...
- [x] Any real failure names the node + real phase, never "extraction failed" — *MPI-387 C* ← **NOT EXERCISED, and formally ACCEPTED as unit-tested-only when MPI-387 closed 2026-08-01.** Three clean installs across two machines never produced a fatal failure for it to describe; proving it means manufacturing a broken environment for a message-only change. `tests/install-path-depth.test.cjs` pins that a pip-only batch never claims "extraction failed". Not pending on this umbrella
...
## Close-out — DONE 2026-08-01

- [x] Every result written into its OWNING card's `validation.md` — MPI-369, MPI-387, MPI-370, MPI-419, MPI-198, MPI-406/407/408/411/413/414/415, MPI-418
- [x] Each owning card moved ← MPI-369 and MPI-387 closed to `done` 2026-08-01; MPI-419, MPI-418, MPI-414, MPI-415, MPI-411, MPI-198, MPI-370 already `done`
- [x] Deferred items recorded with a reason (see `brief.md` § E)

**This umbrella holds no work of its own any more.** The 1.3.0 sweep ran across all
three platforms and shipped. Everything still unticked above belongs to a card that
already owns it:

| Left open | Owner | Why it is not this card's problem |
|---|---|---|
| One model per family, macOS + Linux | **MPI-249** | The card exists to be batched across future releases. macOS generated (SDXL Realistic, then re-validated on build #5); the per-family sweep is a bigger job than a release gate |
| A completed Linux generation | **MPI-249** | Blocked on hardware the project does not have — the only Linux box is a 2011 i3 that thermally shuts down. Confirmed with the user 2026-08-01. Not schedulable work |
| Stalled-download self-heal | **MPI-291** | Opportunistic by design — "never seen fire; do not force it" |
| TAESD preview weights | **MPI-420** | Carded 2026-08-01 |
| Windows update path at 1.4.0 | **MPI-422** | Carded 2026-08-01 out of MPI-387 |

**Standing lesson, worth more than the checklist:** a green checklist is not a
validated release. Build #4's macOS leg passed every automated signal — logs,
timings, file size, gallery card — on an image that was uniform grey noise. Only
opening the file caught it. Any future generate-smoke box means LOOK AT THE PIXELS.
