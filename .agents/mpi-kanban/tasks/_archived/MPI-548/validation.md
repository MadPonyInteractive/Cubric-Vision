...
1. `lora_missing_remote` is emitted only from `comfyController.js:1658` (a `/prompt` 400
   `value_not_in_list` from the Pod) and `partialValidationError` at `:164` (the 200-ack
...
### Fabio's own run — VERIFIED 2026-08-18

> "I verified your implementation. It passes." — Fabio, 2026-08-18, in his own app.
> That is the `user-ux` gate this card was `validating` on. Card closed on it.

The cases he checked:
...
6. **User LoRA present locally, absent on the Pod, cloud dispatch** → still uploaded from
   local disk and the gen runs (MPI-82 semantics preserved, not regressed into a
   Pod-only check).
7. **User LoRA absent locally, cloud dispatch** → still blocked. "Present locally" is the
   requirement in both modes.
8. **Model Settings dropdown reflects the effective engine** — flipping "Run locally"
   re-derives the option list and the "missing" styling, same as it already re-derives
   install state.
9. **Subfolder LoRA, Windows local dispatch** → still resolves (separator regression,
   `routes/comfy.js:999-1001` + `comfyController.js:92-95`).
10. **Empty asset list** → `_findMissingModel` still fails OPEN, and the
   `lora_missing_local` backstop (`commandExecutor.js:2184`) still catches it at the loader.
11. `node --test "tests/*.test.cjs"` green.

Evidence to capture for case 6: `clientLogger` must show the op gate blocking, and
