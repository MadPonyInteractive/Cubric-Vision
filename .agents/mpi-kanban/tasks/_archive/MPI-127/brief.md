...
1. **`generate_ltx.py` handler** + `("LTX_", "ltx")` in `comfy_workflows/scripts/workflow_generation/registry.py`. Produce `LTX_i2v_t2v.json`+`_stage2` (multi-stage `_ms`, Finish-only, `allowsBranchingContinue=false`, no per-stage LoRA variance). Title-keyed: workflow has `Stage1_Bypass` (#70) + `Input_Is_Continue` (#71) — reuse the WAN bypass-one-sampler shape if it fits, else encode LTX's own splice. Verify against a hand-authored stage-2 (the WAN proof method). See workflow_generation/README.md.
   - **SOURCE FILENAME LOCKED (2026-06-24):** `LTX_i2v_t2v_template.json` — the API export is ALREADY in the workflow-gen folder + mirrored in user's G/D folders under this exact name. **DO NOT RENAME** to `LTX23_*`. User keeps the descriptive name across all three locations; future LTX siblings will be separate files (e.g. `LTX_v2v_lipsync`, `LTX_v2v_extend`), so the family prefix is `LTX_`, not `LTX23_`. → registry prefix is `("LTX_", "ltx")`; the brainstorm's per-op-file fan-out (Approach A) still holds — handler fans the one source into per-mode output files, but the SOURCE keeps its single descriptive name.
   - **i2v vs t2v split:** brainstorm settled on **Approach A** — `generate_ltx.py` stamps `Input_Text_to_video` (the MpiBoolean gate, baked per output) to fan ONE source into `LTX_i2v.json` (t2v gate off) + `LTX_t2v.json` (t2v gate on), then WAN-style stage-2 derivation on each. App injects nothing mode-related; op→file map encodes the mode.
   - **FF/LF (first-last-frame) — RESOLVED in MPI-4 (see `MPI-4/research/flf-addguide-splice.md`).** Wave bug was the WRONG node (`LTXVImgToVideoInplaceKJ` multi-image), now replaced with the official two-`LTXVAddGuide` chain (frame_idx 0/-1), gated by `Input_Use_End_Image` on BOTH stages. FL is NOT a separate `Input_Mode=3` file — it's a boolean modifier (`Input_Use_End_Image`) on the i2v workflow, exactly like WAN's `use_end_image`. So per-op files = i2v + t2v only; FF/LF rides the i2v file's gate.
