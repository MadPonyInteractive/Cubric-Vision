...
1. **Add dep + smoke** → `npm i mediabunny`; standalone check: load combined_003 via Input+CanvasSink, `getCanvas(n/fps)` for n=0..19, assert 20 distinct canvases (hash) → **verify: distinct frames, exact.**
2. **Frame-sink service** — `js/services/frameSink.js` (or a manager): wraps Input/CanvasSink lifecycle keyed by URL; `getFrameCanvas(url, frameIndex, fps)`; dispose on video change. → **verify: returns correct frame; disposes cleanly (no leak on repeated loads).**
