...
- **A registered job that never settles kills its lane FOREVER** (`commandExecutor` `_failBail`, MPI-463): `generationStore.register()` takes a **lane slot**, and `_laneBusy` is *derived* from the store's `running` list (INV-6) — so from register() onward, every exit path owes the store a terminal. Eleven pre-dispatch bails (op-not-installed, arch weight, workflow resolve/fetch, trimmed-video prep, missing LoRA, empty media slot, model-not-local, injector, input defaults, preview-latent staging) called `exec.onError` and returned without one, so `_dispatchNextCue` skipped that lane until the app restarted. It does not read as an app bug: the next gen sits on **QUEUED** while the ENGINE queue is EMPTY and the server log is silent — i.e. exactly like a dead ComfyUI. Fixed structurally: **`_failBail` settles to `PHASES.ERROR` then reports, and every pre-dispatch bail routes through it** — the twin of the post-dispatch catch, which already settles once at its own top (that is why ITS nine `exec.onError` branches are correct). Pinned by `tests/lane-settle-on-bail.test.cjs`, which bounds the region by its two anchors and fails on any unsettled bail. **When you add a bail there, call `_failBail`** — that is the whole point of it existing. **And `_failBail` REPORTS by default** (MPI-461): freeing the lane was only half of it — four of those bails settled in silence (workflow-name resolve, trimmed-video prep, workflow JSON fetch, injector throw), so the card just vanished with no toast, no dialog and nothing in `app.log`, which is what got reported as a dead button. It now always `clientLogger.error`s and, unless the site passes `{ reported: true }`, emits `ui:error` through the same `_formatWorkflowError` the post-dispatch catch uses. Pass `{ reported: true }` **only** when the site already emitted its own actionable copy (install this operation, re-attach the media, pick another LoRA) — a test fails an opt-out with no user-facing emit near it.

- **The per-gen engine override is resolved ONCE, in the funnel — never re-threaded from a call site** (`generationService`, MPI-548). `state.engineOverride` is the single source of truth (R31), but `forceLocal` used to travel as an `opts` field each dispatch site had to remember to pass, and **8 of 10 did not**: Continue's stage-2 branch and its cold stage-1 rerun, `preview:finish`, the three group-history tools, `agentDispatch`, `describeAction`, plus `MpiToolOptionsResize`. With "Run locally" ON those paths still froze `engine = 'remote'`, ran the hot-store preflight and let the Pod reject a baked LoRA — and the Cue chip read REMOTE while the toggle read local, because `_buildQueueDisplay` reads the same flag. Now `enqueueGeneration` derives `opts.forceLocal ?? (state.engineOverride === 'local')` **before** building the display, and `startGeneration` repeats it for a direct call. `??` not `||`: an EXPLICIT value must win, because the loop re-fire pins `forceLocal: lane === 'local'` in `_onLaneDrain` and a mid-loop toggle would otherwise bounce it to the other lane (MPI-213). **So a new dispatch site passes nothing** — and the one site that bypasses the funnel (`MpiToolOptionsResize`, direct `runCommand`) reads `state.engineOverride` itself. Threading a routing flag by hand is what produced this bug over ~10 months; don't reintroduce it for the next one.
