...
- `MODELS`: Array of `ModelDef` objects. **A model is ONE install unit** — `{ id, name, mediaType, supportedOps, workflows, dependencies[], dropdownMeta? }`. Every op it declares ships with it; an op it can run is an op whose weights are on disk.
  ...
  - `minVramGb`: Optional per-model floor for the VRAM↔RAM trade table, overriding the computed one (`footprint.js`). Use it only when a model is MEASURED to run below the fit — both H3 cards set 12 against a 13.3 fit. You do NOT need it merely because the fit lands off a round number: the computed floor is lifted onto `CARD_SIZES` (8 / 12 / 16 / then 8s), so a 8.29 fit floors at 12 on its own. An override off that ladder draws its own leading row; the table body always steps by 8.
...
- `getAvailableCommands(mediaType, model, ctx)`: Filters commands by model's `supportedOps` and input availability. Returns `{ key, available, ...CommandDef }` including `components[]` for each command. When `ctx.installedOps` (a string[] from `deriveInstalledOps`) is supplied, a selectable op the user did NOT install is hidden — a T2V-only Wan install never offered I2V (that
example died with MPI-470's t2v deprecation; the filter stands for the next multi-op model). Absent `installedOps`, falls back to static `supportedOps` (no change for image / pre-check models).
...
**Media slots are ADDRESSABLE BY INDEX and SPARSE — never pack them (MPI-306, `e2b1780b`):** a media slot `items` array is indexed **by slot**, a hole is a legal empty slot, and an item's role is its **slot's** role — never its array position. Read a slot = index into it; fill one = assign at its index; remove one = leave a hole (or null it), **never `splice`**. Any `.filter(Boolean)` / `.flat()` / re-index on a slot array *before* roles are resolved is this bug returning. The old packed model put an image dropped on slot 2 into slot 1 — for Head Swap that made the reference image the target and **the swap ran backwards, silently, with a plausible-looking result.** Packing feels harmless because the array stays "valid"; the corruption only shows where position carries MEANING, and then it yields a wrong result rather than an error. The role→graph-param mapping belongs to the APP's controls component via `getInputs({ stepValues })` — the frame collects `{[role]: value}` and must never learn what a role means.
...
**Ordinal vs semantic slot roles (MPI-330):** a `mediaInputs` slot marked `ordinal: true` (qwenEdit/krea2Edit `inputImage`/`inputImage2`/`inputImage3`) is a POSITIONAL alias — the chip's strip order IS the meaning, mirrored by the numbered chip badge. Because `_saveMedia` persists the DERIVED role, restored/reused chips carry a sticky ordinal tag; removing a non-last chip leaves survivors tagged for the WRONG ordinal, and the role-first assignment then strands the required first slot (Qwen-Edit's `block_if_empty` `Input_Image` → ExecutionBlocked → silent zero-output). `stripOrdinalMediaRoles(slots, items)` (commandRegistry) drops ordinal role tags before assignment at BOTH twins (`_withAssignedRoles` + `commandExecutor._buildParams`), so positional fill re-derives them by order. Semantic roles (NO `ordinal` flag — `startFrame`/`endFrame`, Head Swap `image1`/`image2`) stay sticky: those slots mean something, never repack. `commandExecutor` also guards dispatch — a required slot left empty while matching media is attached raises a `ui:warning` toast instead of a vanishing card.

**Chip count is op-declared; the op-switch is count-driven; eviction drops the LAST chip (MPI-292):** the number of PromptBox chips is 100% data-driven from a command's `mediaInputs[]` — never hardcoded in `MpiPromptBox` (`_maxMediaForOperation` counts slots by `mediaType`), so a 2nd reference image is a slot declaration in `commandRegistry.js`, e.g. `{key:'inputImage2', title:'Input_Image_2', required:false}`. Two traps come with it. (1) **`_buildParams`' fallback loop needs its `usedIds` guard** — without it ONE dropped image duplicates into `Input_Image_2`. (2) **At capacity, eviction removes the LAST chip (`afterRoleDrop[len-1]`), not the first** — chip 1 is the edit BASE and must stay sticky. The op-switch rule is **directional**, not count=op-pure: `_opForMediaCount(mediaType, n)` picks the smallest `supportedOp` whose image-slot max ≥ n, so 0→1 promotes t2i→i2i and 1→2 promotes i2i→edit (gated to models that HAVE a ≥2-image op); removing chip 1 of 2 promotes chip 2 and **stays** on edit; →0 falls back to t2i.
