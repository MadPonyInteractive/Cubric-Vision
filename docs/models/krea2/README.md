...
| Workflows | **ONE file per card** since MPI-365: `krea2_t2i_<sfw\|nsfw>.json` serves all seven ops, branch picked by `Input_wf_type` (1 t2i · 2 i2i · 3 depth · 4 edit · 5 inpaint · 6 detail · 7 upscale). `krea2_detailer_*` / `krea2_upscaler_*` are **deleted** — see [injection.md](injection.md) |
| Ops | `t2i` · `i2i` · `control` · `krea2Edit` · `inpaint` · `detail` · `upscale`. Edit takes an **optional mask** (empty = whole-image) and re-renders the whole crop; `inpaint` (MPI-615) is the branch that holds everything outside the mask still. `control` is the op formerly called `poseReference`, then briefly `depth`; Krea2 declares `controlTypes: ['depth']`, so it shows no type picker |
