...
| [windowing.md](h3/windowing.md) | **`MpiWindowedSampler` and the stage-2 tier table.** Which canvases sample the refine in temporal windows and which do not, how `Window Frames` derives the ceiling from the stage-2 target size, why `overlap_frames` is one constant at 17 (9 and 17 plan identically; 4K ignores it entirely), the `17k+5` / `5k+2` grid a window must not cut across, and the TWO distinct memory walls — attention vs MLP — with what is measured and what is not. Read before changing a window number or reading an H3 OOM. |
