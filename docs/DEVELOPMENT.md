...
- **But for BOOT evidence, grep the whole `logs/` DIRECTORY — `app.log` alone is a false negative** (MPI-459). A cold boot writes ~250 KB (the ComfyUI node scan), so it crosses the 256 KB cap *during* startup: everything before the rotation is already in an archive and `app.log` holds only the tail. Verifying a boot-time code path by grepping `app.log` returned zero hits on a path that had run — the three proving lines were in the archive, 15 s ahead of the ones that survived. Use `grep -riE "<pattern>" "$APPDATA/Cubric Vision/logs/"`. (To *drive* log volume through the real relay rather than waiting on it, `POST /log` — `routes/system.js`'s client bridge — writes through the server fork's logger; ~320 lines of ~250 chars crosses the cap.)
...
**A dead stdout is not a boot crash, and used to look exactly like one (MPI-514).** Whenever the thing reading main's stdout goes away — an agent piping `npm run app:isolated` into `head -1`, a killed launcher, a closed terminal — the console mirror in `routes/logger._write` fails ASYNCHRONOUSLY: the call returns, the stream emits `'error'` afterwards, so that function's own try/catch (written for a synchronous EIO) never sees it. Node then re-raised it through `uncaughtException` and a modal "Cubric Vision failed to start" box appeared, quoting an EPIPE stack, on a healthy app. `main.js` now attaches `'error'` handlers to `process.stdout`/`stderr` that swallow `EPIPE`/`EIO` only; every other stdio error still goes to `reportFatal`. The file log is the durable sink, so the dropped mirror costs nothing. Note the electron npm CLI wrapper has the same EPIPE fragility and is third-party — piping a launch into `head`/`grep -m1` still kills the app, just quietly now.

Still invisible: anything that kills the process before those handlers register (a require-time throw in `main.js`'s first nine lines), and anything that prevents Electron from starting at all. For those, ask the user to run the app from a terminal — `CubricVision.exe` from a console on Windows (MPI-387 deleted the `.bat`/`.vbs` launchers), `start-with-terminal.sh` on Linux — because main-process stdout never reaches any log file.
