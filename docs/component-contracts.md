...
## MpiLoraRack — one value, two views, and neither may subscribe to `state:changed` (MPI-724)

The PromptBox settings popup and `MpiModelSettings` both render the SAME `modelSettings[modelId].loras`. The overlay owns the SET (which LoRA sits in which slot — the only place one can be added, removed or re-pointed, per MPI-356); the rack owns nothing and only adjusts strengths and bypasses. Three contracts hold them together:

- **One definition of a row.** `Compounds/MpiModelSettings/loraSlotParts.js` exports `buildStrengthsRow` / `buildBypassBtn` / `applyBypass` plus the path helpers, and both surfaces build from it with their own `bem` prefix. The bounds (`-2..2`, step `0.05`, 2 decimals) and the bypass contract (`aria-pressed` is the only source of truth, MPI-588) have to agree byte-for-byte — the two views write the same key, so a drift between them shows up as a number that changes when the user looks away.
- **Sync is `settings:model:update`, filtered, with a self-echo guard.** Each side listens for `key === 'loras'` on its own `modelId` and sets a `_selfWrite` flag around its own `Events.emit`. That works as a plain boolean *because `Events.emit` is synchronous* (`events.js`) — every listener has run before `emit` returns. **Do not reach for `state:changed` instead**: `projectService` debounces the write ~300 ms into `state.currentProject`, and a listener on that key re-enters on its own echo — the same loop that made the overlay unclosable in MPI-356 (see above).
- **Apply in place; rebuild only when the SET changes.** An incoming value edit is written through the held `MpiInput` handles (`el.setValue`) and `applyBypass`, never by remounting: a remount tears down an open `MpiTreePicker` mid-search on the overlay side and steals focus from a half-typed strength on the rack side. The rack compares a `stageKey:index:name` signature and rebuilds only when that differs — which only the overlay can cause. This is why `buildStrengthsRow` returns `{ el, modelInput, clipInput }` rather than a bare element.

- **The event stream is AHEAD of the project for ~300 ms, so a re-read inside that window LIES.** `projectService._enqueueModelUpdate` debounces `_QUEUE_DEBOUNCE_MS = 300` and assigns `state.currentProject` only inside the timer — the disk write and the *state* write are the same deferral, not two. Measured 2026-09-11 on the first build of this rack: emit LoRAs, open the cog in the same tick, and the popup came up **empty**, because its open handler called `refresh()` and read a project that had not caught up. Two rules fell out of it: the popup's open path does **not** re-read (the rack's listener is subscribed whether the popup is shown or not, so an overlay edit made while it was shut has already arrived), and every re-read goes through a guard that keeps the held value while this rack's own write is still in flight. Without that guard the second half of the same bug fires from `_refreshOpSlot`, which runs on **every op change**: pick an op within 300 ms of editing a strength and the stale project value would silently revert the number the user just typed.

Two smaller rules the rack carries: the filled-slot filter keeps each slot's **original index**, because a write has to land back in its own position and never in a compacted one; and an empty `state.availableLoras` means the scan has not run, not that every file is gone, so the missing-file flag is suppressed until the list is non-empty.

`refresh()` is not dead API: **Reuse Prompt writes `loras` straight into the project with no event at all** (`projectService.applyPromptReuseSettings` → `setModelSettings`), and reaches the rack through `el.refreshControls()` → `_refreshOpSlot` → `setModel`. It is the one writer of this key that the sync listener cannot hear.
...
## MpiButton — `label` belongs to ICON mode; text mode renders `text` and nothing else (MPI-664, 2026-09-11)

`MpiButton` is two components behind one name and the switch is `const isIconMode = !!props.icon` (`MpiButton.js:33`). **Text mode renders `props.text` only** (`:45`) — `label` is never read on that branch. **Icon mode renders `props.label`** beside the icon (`:90`) and ignores `text`. So `MpiButton.mount(host, { label: 'man (Male)' })` with no `icon` mounts a real, correctly-classed, correctly-listening, **EMPTY** `<button>`: `textHtml` is `''` because `text` is undefined, and nothing throws, warns or logs. It shipped to the user as a picker of three zero-height rows.

Both working call sites dodge it by accident and **neither is transferable**: the voice roster's rows pass `icon:'plus'` *alongside* `label` (icon mode, so the label renders), and `MpiPromptBox` passes inner HTML as `children` (text mode renders children, so `text`/`label` are both irrelevant). Copy either into a plain text button and you get the empty button. **Text button → `text`. Icon button → `icon` + `label`.** See also *MpiButton icon mode REWRITES `variant`* above — the same mode switch silently rewrites the variant.
...
## MpiAudioRecorder — a recording is not an import, and the container is not the browser's (MPI-573)

`showAudioRecorder()` → `Promise<File|null>`, mirroring `showLicenceGate()`. Three things about it are non-obvious and each was a real trap.

**(1) It re-muxes to WAV, and must.** MediaRecorder on Chromium produces `audio/webm;codecs=opus` (measured in the app, 2026-08-22 — not assumed), and `.webm` is classified as **video** by extension in five server lists (`mediaTypeFromExt`, the two reconciler paths at `routes/projects.js:1008`/`:2722`, the media-scan ext set, `js/utils/file.js` AUDIO_EXTS). The upload sidecar would say `audio` and the first project reload would silently re-type the card to video. Chromium's MediaRecorder cannot emit Ogg either, so the blob is decoded and re-encoded by `js/utils/wavEncoder.js` — pure, import-free, and unit-tested against the real ffprobe in `tests/audio-media-type.test.cjs`. **Clamp before scaling to int16**: input gain can push a sample past 1.0, and an unclamped overflow wraps to full-scale NEGATIVE, so a loud take comes back as crackle instead of clipping.

**(2) The mic card in `MpiMediaPicker` does NOT route through `onImport`.** A recording exists nowhere else, so it goes the gallery-drop route — `uploadMediaFile(file,'audio',…)` then `Events.emit('media:imported', …)` — and only then resolves as an ordinary `pick`. Routing it through `onImport` would fill the slot via the preview-asset store and leave no gallery card, which is the opposite of the point. The card is gated on the **slot's** type, not the active filter tab.

**(3) Installing a permission handler at all inverts Electron's default for everything else.** `getUserMedia` needs `session.setPermissionRequestHandler`, and with none installed Electron was granting every permission by default — including the two the app already relies on, both of which fail silently: `fullscreen` (video control bar, focus mode) and `pointerLock` (`MpiRadialMenu`, the operation picker). All three are in the allowlist in `main.js`; desktop notifications are **not** and must not be — they are raised by the main process and never ask the renderer's session. `setPermissionCheckHandler` is the synchronous twin and is what makes `enumerateDevices` return real device **labels**, without which the Settings picker lists identical blank rows. `tests/desktop/audio-permission.spec.js` guards all of it.
...
**Hovering an audio tile plays it, and the `<audio>` must NOT carry `data-src`.** `_stopOtherGalleryMedia` in `MpiGalleryGrid` selects `audio[data-src]` across the whole **document** on every gallery scroll event, and `MpiModal` portals this picker to `document.body` — so marking these elements the way the gallery marks its own hands the grid behind the modal a remote control over the picker's playback. The picker tracks its one playing clip in a variable instead. Two more details, both borrowed from the gallery's audio card: the element is built on the **first hover**, not with the tile (at `preload='metadata'` one per tile is a fetch for every clip in the project the moment the Audio tab opens), and volume is `Storage.getGalleryVolume()` where **0 IS the mute** — hover-play is skipped outright, because playing at zero would still swap the glyph to Stop and lie. Reset the glyph on `ended` as well as `pause`: `pause` does not fire when a clip runs out.
