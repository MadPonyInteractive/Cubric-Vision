...
**Drain signal = `state.generationQueueCount === 0`, NOT `generationStore.depth`.** The cue queue (`generationService._cueQueue`) feeds the store ONE job at a time, so the store's own depth hits 0 between every item → keying on it fires per-gen. `generationQueueCount` = `_cueQueue.length + running` is the true whole-batch depth. **Flush is deferred one tick** (`setTimeout 0`): the last item's `generation:complete` and the count reaching 0 fire from decoupled paths in either order, so a synchronous flush can miss the final increment; the deferred flush re-checks the count is still 0 (guards a fast re-cue).
