...
### Completion toasts COALESCE; in-app toasts are SILENT

Per-gen completion feedback is NOT a per-gen toast. A queue of N gens fired N toasts (and while minimized they piled up frozen — transitions/rAF don't run hidden — then flooded on restore). Instead: `notificationService` (in-app path, when the app is focused) calls `StatusBar.notifyCompletion()` which just **increments `_doneCount`**; one summary toast — "N generations finished." — fires when the queue drains (`generation-store:changed` with `depth === 0`, in `statusBar.js listen()`). Focused or not. `progress.complete()` no longer mounts a per-gen toast.

Sound: **in-app toasts never chime** — `MpiToast`'s `_playToastSound`/`getToastSound` chime was removed. The "Play sound on notification" setting (`getToastSound`, key `TOAST_SOUND`) now gates the **OS/background notification** sound only: `notificationService` passes `sound: Storage.getToastSound()` in the `notify-*` IPC payloads, and `main.js showOsNotification` sets `silent: payload.sound === false`. The OS path fires only when `!document.hasFocus()` (not focused, covers minimized). Setting copy matches: "Fires only while the app is not focused. In-app messages are always silent."
