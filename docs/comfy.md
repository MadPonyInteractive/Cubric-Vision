...
**Engine bootstrap retry contract (MPI-8, corrected MPI-414):** `_clearStaleWindowsEngineArtifacts()` runs BEFORE download — removes partial archive, OS-renamed `(n)` dups, partial extract folder with no Python. Post-extract: assert `getPythonBin()` exists; else scrub + throw. **Retry routes by `/engine/version-check`:** `installed:null` → `/engine/download`; `installed:"<ver>"` → `/engine/repair-deps`. It used to route by `/engine/status`, which only answers "does the venv python exist" — true from step 1 of the uv path (Linux/macOS), long before ComfyUI is cloned, so an interrupted first install sent Retry to deps-only, which reported success and left an engine that dies on `ModuleNotFoundError: sqlalchemy`. **The version stamp is the ONLY completeness marker** (written after a successful `comfy install`, and version-check deletes it if the python is gone), so `/engine/repair-deps` now refuses an unstamped engine and hands off to `_runEngineDownload()` rather than broadcasting `engine:complete` over a dead engine. `/engine/status` survives with no live consumer. Never stamp from a path that did not run `comfy install` — a green stamp on a broken engine is worse than no stamp.

**macOS torch is PINNED BY US, never by comfy-cli (MPI-419, 2026-08-01):** comfy-cli's `MAC_M_SERIES` branch is the ONLY one of its GPU branches that installs from the PyTorch **nightly** index, and it does so with `--pre` and no version — so the engine a Mac user got depended on the date they installed. It shipped garbage: nightly `2.14.0.dev20260731` renders SDXL on MPS as uniform grey noise **silently** — no error, normal ~74s runtime, normal gallery card. Isolated with ComfyUI held at 0.29.2: dev20260731 = noise, dev20260730 = correct, stable 2.13.0 = correct. Windows was never exposed (torch frozen inside the prebuilt portable archive, `2.13.0+cu130`); Linux resolves stable from PyPI (`whl/cu*`, `whl/cpu`). Fix: `dev_configs/system_dependencies.json` → `torchMac`, installed at `_provisionUvEngine` **step 2b BEFORE comfy-cli runs** (so ComfyUI's `requirements.txt` sees torch already satisfied) PLUS `--skip-torch-or-directml` at step 3 (so the nightly branch never executes). Both guards are deliberate — either alone still leaves a path to a nightly wheel. **Standing lesson: pinning ComfyUI was necessary and not sufficient; an unpinned dependency anywhere in the install path means the build we test is not the build the user gets.** Linux torch still floats (stable channel, lower risk) — uncarded.
...
- The carrier identifies the engine — **on a 400 only** (see the 200 bullets below): `node_errors` ⇒ local, `detail.comfy_body` ⇒ remote. Tag the thrown error accordingly (`lora_missing_local` / `lora_missing_remote`) so `commandExecutor` can raise a `ui:warning` toast rather than letting it fall through to the `ui:error` GitHub bug-reporter dialog.
- **A 200 is NOT proof the graph will run — `node_errors` rides along on SUCCESS (MPI-495).** ComfyUI validates per OUTPUT node and fails the *request* only when EVERY output is invalid (`validate_prompt` returns `(True, …)` as soon as one survives, `execution.py`). With one good output it QUEUES the prompt, answers `200 {prompt_id, number, node_errors}` (`server.py`), logs `Output will be ignored`, and executes the rest of the graph. `node_errors` is present on **every** 200 — `{}` when the prompt is clean, so test its LENGTH, never its truthiness. Reading only `prompt_id` is what delivered an image with all ten krea2 style LoRAs silently dropped, reported as a complete success with no toast and nothing in `app.log`.
- **On a 200 the carrier canNOT name the engine.** The wrapper forwards a 200 verbatim (`JSONResponse(r.json())`) and so does Express `_passthrough`, so REMOTE carries structured `node_errors` too. Tag by the engine (`_alwaysLocal` / `remoteEngineClient.isRemote()`), or a remote drop shows the local toast's "add it in Settings" advice.
- **A style rack's slots are `lora_1..lora_5`, not `lora_name` (MPI-359).** `findRejectedFile(nodeErrors, ['lora_name'])` matches NONE of an `MpiStyleLoras` bank's rejections — collect the rejected `extra_info.input_name`s by shape and pass those in.
- **Both carriers are read by ONE helper — `js/utils/comfyValidationError.js` (`findRejectedFile`), and its `carrier` field IS the engine tag.** Adding a new rejection family = one more call with a different input-name list, never a second parser. Two families exist: `lora_name` (MPI-229) and `MODEL_FILE_INPUTS` = `unet_name` / `ckpt_name` / `vae_name` / `clip_name` (MPI-453) → `weights_missing_local` / `weights_missing_remote`. The weight family means the user never installed that operation, so the toast names the file and points at the Model Library.

Anything that classifies a ComfyUI rejection MUST handle both shapes — see the "fix BOTH engine paths" law. Guards: `tests/lora-missing-local-toast.test.cjs`, `tests/uninstalled-op-gate.test.cjs`, `tests/prompt-partial-validation.test.cjs` (the 200 case, driving the real `runWorkflow`). **Every consumer of a `/prompt` ack owes the same check** — `scripts/smoke-workflows.mjs` scored PASS on media count alone and reported a fleet of unstyled generations as green until MPI-495/467 fixed both readers.
