...
The pod Dockerfile's weight-prebake `RUN` (bakes rife47 / 4x upscalers / face_yolov8n / sam_vit_b via `aria2c -x16 -s16`) intermittently FAILED the whole `docker build` with `aria2 errorCode=22 … status=403`. Cause: HuggingFace migrated those repos to its **Xet CDN** (`us.aws.cdn.hf.co/xet-bridge-us/…`), whose signed-URL policy 403s ranged/segmented aria2 requests non-deterministically — one GPU profile survived (aria2 retried into a good range), a parallel one didn't. Fix: point the 4 non-RIFE weights at **R2** (`https://models.cubric.studio/vision/models/<type>/<file>`) — the SAME source `dependencies.js` already uses (MPI-129/178) and the SAME bytes (SHAs unchanged). RIFE stays on HF (`marduk191/rife` was a plain non-Xet repo). **STALE as of 2026-08-08 (MPI-491): HF finished the migration and `marduk191/rife` now 302s to `us.aws.cdn.hf.co/xet-bridge-us` like everything else — verified by `curl -sI`. There is no non-Xet HF repo left to rely on, so the RIFE prebake carries the same 403 risk the other four were moved off. Either mirror it to R2 too or expect the build to flake.** The weights are ALSO on local disk at `G:/CubricModels/` (see docs/builder/01-environments.md) — no re-download needed to re-mirror. RULE: Dockerfile weight-prebake urls MUST mirror `dependencies.js` (both R2); never (re)introduce a `huggingface.co` prebake url — a future HF Xet migration re-breaks the build. If a build 403s on a weight: confirm it exists on R2 — but **NOT with `curl`. Re-measured 2026-08-08 (MPI-491): a SINGLE `curl -sI` against `models.cubric.studio` returns `http=000` with exit 43 while the object is served fine** (node `fetch` got HTTP 200 and read 939MB at 47 MB/s from the same box, same minute). The old advice here said a single call was the reliable form and only a looped `curl -o` lied; that is wrong, and following it makes a healthy R2 object look missing — the precise false negative this note exists to prevent. It is host-specific, not global: `curl -sIL` against `huggingface.co` answers correctly on the same box. Verify R2 with node `fetch`, or with `rclone lsl`, and repoint only after that.
