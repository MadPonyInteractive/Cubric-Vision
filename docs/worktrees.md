...
> Project name: **Cubric Studio**. Root folder is currently `MpiAiSuite/` for legacy IDE-link reasons; paths in this doc reflect the on-disk folder name.
...
```json
{
    "enginePath": "C:\\AI\\Mpi\\MpiAiSuite\\engine",
    "llamaPath": "C:\\AI\\Mpi\\MpiAiSuite\\llama_engine",
    "llamaModelsPath": "C:\\AI\\Mpi\\MpiAiSuite\\llama_models"
}
```
...
1. `git worktree add ../MpiAiSuite-feature feature-branch`
...
3. `cd ../MpiAiSuite-feature` and launch — engine/models read from main worktree
...
```bash
# create worktree
git worktree add ../MpiAiSuite-myfeature myfeature

# list worktrees
git worktree list

# remove worktree
git worktree remove ../MpiAiSuite-myfeature
```
