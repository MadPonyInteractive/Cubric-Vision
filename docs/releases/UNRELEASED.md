...
- **Notification sound** — a sound now plays when something finishes that you weren't watching for — a generation completes, a model finishes installing, the engine restarts. It fires only while the app is not focused, as a desktop notification; in-app messages stay silent. Toggle it in Settings under Notifications.
