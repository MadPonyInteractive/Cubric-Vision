# Cubric Vision v1.5.0 — 2026-09-07

## Changelog

A feature release built around MiniMax H3 — faster, sharper at 2K and 4K, and far more
reliable on a rented GPU — carried by an engine bump from ComfyUI 0.31.0 to 0.34.0. It
also brings across the non-Flow image-model work: a second FLUX.2 Klein (the 9B) and
inpainting on the SDXL family and Krea 2.

Cut from the last released tag as a deliberate deviation from the 2.0 roadmap, so this
work reaches users without waiting for Flows. Flows, Music Maker, Stable Audio, the 3D
scene and the gallery rendition ladder all stay on the 2.0 line.
...
None. `inpaint` reaches five SDXL-family cards and both Krea 2 cards here, but the
operation itself already existed — on FLUX.2 Klein alone.

### Breaking Changes

None. `SCHEMA_VERSION` stays at 4; no project migration is required.

### ComfyUI Engine

Updated 0.31.0 → 0.34.0.
...
The released Pod image moves with it (`POD_IMAGE_VERSION` / `POD_IMAGE_VERSION_CPU`
v0.21.0 → v0.23.0, CI run 34163882224, both legs). That pairing is mandatory rather than
tidy: this release's graphs carry `ModelAttentionBackend`, a CORE node absent from
0.31.0, so a 1.5.0 app against a 0.31.0 Pod rejects every remote H3 and LTX run.

Smoke evidence: `dev_configs/smoke-evidence.json` — engine proven 0.34.0, **44 ops,
0 FAIL**, across two merged runs (the full 37-op matrix, then a scoped `--models
klein-9b` pass, because the matrix dedupes 9B behind the cheaper 4B and would otherwise
leave the 9B transformer and all seven 9B style LoRAs unloaded). `release:check` reports
coverage of all 21 models — 13 run, the rest by class_type dedupe.
...
- [ ] **GitHub Releases**: Create release with tag v1.5.0, attach portable artifacts (gated per repo-distribution rules), write release notes
