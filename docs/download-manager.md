...
**Who decides changed in MPI-500 (`d3d475a9`): the bin is now a Settings toggle, and it is OFF by default.** Both delete sites go through one `_removeManagedFile(p, useRecycleBin, …)`, and `useRecycleBin` defaults to `false` on the uninstall route and on `_sweepOrphanedDeps` — so a request that omits the field deletes permanently. With the toggle off, which is what every test and agent sandbox gets, NOTHING goes to the bin and the quota is never reached. The quota facts above still hold for the toggle-ON path.
