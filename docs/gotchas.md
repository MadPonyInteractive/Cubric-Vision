...
aimdo's dynamic-VRAM fault-in is **load-bearing**, NOT wasted overhead — do not disable it to chase perf. TESTED LIVE 2026-06-27 on a 4090 (24GB): passing `--disable-dynamic-vram` (start.sh `VRAM_MODE`, R2-pushed) made ComfyUI load LTX-2.3's **full 3-stage** weight set RESIDENT with the offload spec on CPU → it streamed **~57GB into a Pod with ~57GB RAM** → status bar stuck `LOADING MODEL 0%` for 2:54+, RAM 98%, then the **container OOM-killed mid-gen** (app shows "remote engine disconnected — the Pod may have run out of memory and restarted"). Same failure as MPI-146's 5090 `--lowvram` OOM. The "LTX peaks ~18GB so it fits 24GB" reasoning is WRONG: one STAGE fits VRAM, but the full pipeline's resident RAM footprint does not. `--highvram`/`--gpu-only` are MORE aggressive → same/worse. **The Pod-vs-local warm gap (4090 2:09 vs local 4060Ti 1:31) is NOT a VRAM flag and NOT a torch version (cu130 = separate dead end). The remaining lever is the model FORMAT — fp8 native weights load faster WITH aimdo ON** (per ComfyUI's own `--disable-dynamic-vram` deprecation warning + aimdo guidance: *"fp8 will be faster even if larger than your memory"*). Full live-test table + the fp8 next-step in `docs/builder/research/pod-perf-investigation.md`. start.sh default is `VRAM_MODE=""` (aimdo manages); `CUBRIC_VRAM_MODE` env overrides only for ad-hoc tests. NOTE: host throttle was also CLEARED live (4090 = 165 TFLOPS bf16, P0, full clocks under load — GPU starves at 1% util, it does NOT throttle).
