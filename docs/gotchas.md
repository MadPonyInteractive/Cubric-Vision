...
> **★ RESOLVED 2026-06-28 — the "FIX" lines above are SUPERSEDED. Read the top banner of `docs/builder/research/pod-perf-investigation.md`.** The cloud slowness is a ONE-TIME COLD-LOAD TAX, not per-gen. Live on a 5090 Pod: 1st gen (cold) ~209s; **2nd gen onward (warm) = 36s** (1s 1280×704, beats local). The 40GB stays resident after the first fault → every later gen is fast. REAL cold-cause = RunPod's **8MB `RLIMIT_MEMLOCK` cap** (not raisable via RunPod API) throttling aimdo's pinned staging + the per-page fault mechanism. DEAD fixes (all disproven, do NOT re-try): bigger VRAM (96GB still cold-faulted 119s), torch 2.8→2.11 bump (no change), disk (read_bytes=0), `--disable-async-offload` (worse), `--disable-pinned-memory` (no change — the per-page fault, not pinning, is the cost), fp8 transformer (eyes/teeth quality reject), downgrade to 0.25.1 (it ALSO ships aimdo 0.4.10 → same cap on a Pod). TWO real OPTIMIZATIONS (keep bf16 + 0.26): (1) **warm-on-connect** — fire a throwaway gen after Pod connect so the user never sees the ~3:30 cold tax; (2) **remove `MpiClearVram` node 58** from `comfy_workflows/LTX_t2v_stage2.json` (it `unload_all_models()` mid-pipeline → re-faults the 40GB at stage-2 even warm) — UNTESTED, needs a 24GB VRAM-peak check before shipping (Wan has 3 such nodes + is fine, so it may be load-bearing on small cards). Upstream ComfyUI issue #14345 = same LTX-2.3 regression since the aimdo bump in 0.24.
