...
aimdo's dynamic-VRAM fault-in is **load-bearing**, NOT wasted overhead — do not disable it to chase perf. TESTED LIVE 2026-06-27 on a 4090 (24GB): passing `--disable-dynamic-vram` (start.sh `VRAM_MODE`, R2-pushed) made ComfyUI load LTX-2.3's **full 3-stage** weight set RESIDENT with the offload spec on CPU → it streamed **~57GB into a Pod with ~57GB RAM** → status bar stuck `LOADING MODEL 0%` for 2:54+, RAM 98%, then the **container OOM-killed mid-gen** (app shows "remote engine disconnected — the Pod may have run out of memory and restarted"). Same failure as MPI-146's 5090 `--lowvram` OOM. The "LTX peaks ~18GB so it fits 24GB" reasoning is WRONG: one STAGE fits VRAM, but the full pipeline's resident RAM footprint does not. `--highvram`/`--gpu-only` are MORE aggressive → same/worse. **The Pod-vs-local warm gap (4090 2:09 vs local 4060Ti 1:31) is NOT a VRAM flag, NOT fp8 format, and NOT cu130 cuBLAS.** Live-verified BOTH engines load the IDENTICAL model set (BF16 22B transformer + fp8 gemma encoder + bf16 VAEs) with aimdo ON + CPU-offload — and the 16GB local card offloads MORE and still WINS, killing the offload theory twice. FOUR dead ends closed: cu130 GEMM (Blackwell-only), host throttle (4090 = 165 TFLOPS bf16 / P0 / full clocks — GPU STARVES at 1% util, does NOT throttle), aimdo-off (this OOM), fp8 (identical already). The ONLY remaining axis is the **torch framework jump 2.8 (Pod) vs 2.12 (local)** — the cu130 research cleared cuBLAS GEMM gains, NOT the 4-minor-version torch bump (SDPA kernel pick / inductor / CUDA-graph / allocator all changed). NEXT = per-step stdout profile to localize the cost, then a torch-ONLY bump test (check for a 2.12+cu126 wheel to keep the wide driver floor — do NOT blindly take cu130's r580 floor). Full live-test table + plan in `docs/builder/research/pod-perf-investigation.md`. start.sh default is `VRAM_MODE=""` (aimdo manages); `CUBRIC_VRAM_MODE` env overrides only for ad-hoc tests.
