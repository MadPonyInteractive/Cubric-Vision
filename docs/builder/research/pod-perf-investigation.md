...
>
> **The graph is ALREADY single-prompt-capable — the blocker is the two-POST dispatch, not the JSON.** `Input_Is_Continue` (node 71, `MpiBoolean`) already exists in BOTH `LTX_*.json` and `LTX_*_stage2.json`; stage-2 = stage-1 with that one boolean flipped `true` (`generate_ltx.py:_derive_stage2`, lines 136-140). The `Stage1_Bypass` sampler (node 70) stays in both files; `MpiIfElse` gates route loaded-latent (continue) vs live-latent (preview). So the two files are ONE graph with a flag — but the flag was designed for the two-prompt model (load latent from DISK vs live), so a true single-prompt run needs the stage-1 sampler output to feed stage-2 DIRECTLY (kill the Save→Load round-trip) = graph re-wiring, not just flipping 71.
>
