...
> 1. **`--preview-method taesd`** — ours forces it, stock passes nothing. Prime suspect: taesd preview-decode work at/around the stage boundary (interacts with the KJNodes `LTX2SamplingPreviewOverride` LTX preview override) that stock skips. TEST: drop it (or set `--preview-method none`) on the Pod, remeasure.
