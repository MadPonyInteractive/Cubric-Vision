...
> ## ✅✅ MPI-191 SOLVED 2026-07-05 — THE STAGE-GAP TAX IS NETWORK-VOLUME READ BANDWIDTH. WEIGHTS ON THE CONTAINER DISK = 9s GAP. EVERYTHING ELSE BELOW IS ACQUITTED HISTORY.
>
> **Live same-pod bisect (5090 32GB, 190GB-RAM host, v0.12.0 image, bf16, direct-8188 door, ALL other axes held):**
> | weights location | warm stage-1→2 gap | cold full |
> |---|---|---|
> | `/workspace/mpi_models` (NETWORK VOLUME, dd-measured **750 MB/s**) | **36-39s** | 280s |
> | `/opt/ComfyUI/models` (CONTAINER DISK, same pod, same file) | **9s** | 128s |
> | (stock reference, weights on disk) | ~9s | ~60s |
>
> **Mechanism (REFINED by the user's stock-volume counter-evidence, 2026-07-05):** aimdo re-faults the 40GB LTXAV at every stage transition (always true), and each re-fault reads from the fastest tier that still holds the bytes: **page cache → local disk → network volume.** The stock test ALSO had its models on a network volume yet ran 59.79s-cold/9s-warm — impossible via a 750MB/s volume (51GB of weights alone = ~68s > the whole gen). Resolution: the stock models had JUST been downloaded (write = cache fill) on a **386GB-RAM** host → every read was page-cache/RAM-speed; the volume was never re-read. Product pods land in tier 3: models installed in a PAST session (nothing cached) + modest host RAM with aimdo pinning most of it (`Enabled pinned memory 171731` on the 190GB host) → page cache can't hold 40GB → re-faults fall through to the 750MB/s volume. **Disk staging moves us to tier 2 deterministically** (no host-RAM roulette, no install-recency luck). Alternative lever for the fix design: big-RAM hosts via the shipped MPI-160 `minMemoryInGb` floor buy tier 1 probabilistically — dearer than $0.004/hr of disk and evictable on long sessions. On the container disk the same re-fault is ~9s. **Acquitted by direct elimination in this one live session: the wrapper (direct-8188 gen = same 36s), launch flags, dup/stale volume node packs (removed → 38s), ComfyUI 0.27-vs-0.26.2 (in-place rollback → 36s), torch minor, hardware, aimdo.** The earlier "read_bytes=0 → fault is RAM-not-disk" finding was measured on a different setup and does NOT hold for volume pods.
> **Why every earlier test missed it:** the fast stock tests followed the "download test models to the DISK, not the volume" rule — nobody flagged that the PRODUCT pods do the opposite (extra_model_paths points at `/workspace/mpi_models`). Ephemeral pods (MPI-78, `CUBRIC_EPHEMERAL=1`) already root models on the disk — **only volume pods pay this tax.**
> **THE FIX (to implement):** stage hot weights onto the container disk at boot for volume pods — at minimum the LTX transformer (40GB, ~53s copy at 750MB/s), ideally + the 11GB TE. Container disk is 50GB on volume pods (`CONTAINER_DISK_GB`, remotePodLifecycle.js:138) → fits the transformer alone; staging more needs a disk bump in the create payload (cost call). start.sh change + create-payload change, NO image rebuild. Bonus mystery solved: the boot log's TE "staged 3×" and every historic "cloud slower than local" datapoint on volume pods inherit this explanation.
>
> **★ MPI-329 SHIPPED 2026-07-23 — fix landed + the target FLIPPED.** The tax was NOT LTX-specific: EVERY fits-VRAM model (Krea2 13.5 / Qwen 20.5 on a 24GB card) was volume-read-bound the same way, so the shipped fix stages the whole fits-VRAM weight set (`HOT_STORE_MIN_GB` 20→0.1), not just the LTX transformer. TWO REGIMES: fits-VRAM → staging = ~10× (switch 80s→9s, live 4090); a single file > VRAM (LTX 42GB bf16 on 24GB) STREAMS per-stage regardless of source → staging can't help AND its 42GB copy hogs the wrapper's one hot-store lock (stalled an interactive gen for minutes, live). So `_ensureRemoteHotStore` now **SKIPS any file > pod VRAM** — the OPPOSITE of "at minimum stage the LTX transformer" above (that plan assumed the big-RAM host where it fit; on a 24GB card it can't). Container disk is no longer static 50GB `CONTAINER_DISK_GB`: it MIRRORS the network-volume size (+5GB) so the LRU never evicts. Full mechanics: `docs/runpod-remote-engine.md` § hot-store.
>
> ---
>
...
>
> **★★★ THE REAL DIAGNOSIS (2026-07-04, user-driven — supersedes the "inter-prompt eviction" research framing below):** a full (non-preview) LTX gen is **ONE `/prompt`, not two** — `LTX_t2v.json` (single mode, resolved when neither `previewOnly` nor `isStage2` is set) runs BOTH samplers in one graph (node 442 STG1 + node 39 STG2), no load/save latent between them. So the re-fault is **INTRA-prompt**, not inter-prompt. The research agents' "two separate prompts" premise was WRONG for the full-gen path (it's only true for the explicit preview→continue UI flow). **Node 58 (`MpiClearVram` = `unload_all_models()` + `soft_empty_cache()` + `torch.cuda.empty_cache()`, verified in `ComfyUi-MpiNodes/vram.py`) sits between the two samplers and releases the 40GB VBAR pages → that's the re-fault trigger.** BUT node 58 is **NOT the culprit for the app-vs-stock gap** — see next.
> **THE DECISIVE SPLIT (live, same workflow WITH node 58, same LTX_t2v.json):**
> | stack | card | RAM | warm stage-1→stage-2 gap |
> |---|---|---|---|
> | **RunPod OFFICIAL `comfyui-cuda13.0` image** (torch 2.10+cu130) | 5090 32GB | ? | **~11s** |
> | **OUR cu130 image v0.13.0** (torch 2.12) | 5090 32GB | 55.88GB | **43s** (stage-1 end 1:08 → stage-2 start 1:51; stage-2 itself 3s) |
> | **OUR cu130 image v0.13.0** (torch 2.12) | 4090 24GB | 61GB | **~86s** |
> **★★ CONFIRMED LIVE 2026-07-05 — the gap is OUR APP/LAUNCH, NOT the hardware/torch/aimdo. Reproduced on a clean stock Pod.** Ran OUR `LTX_t2v.json` on RunPod's official `runpod/comfyui:cuda13.0` image (5090 32GB, **386GB RAM**, torch **2.10.0+cu130**, ComfyUI 0.26.2, comfy-aimdo 0.4.10 — same stack as our v0.12.0), models symlinked in, the 3 needed node packs (LTXVideo/KJNodes/MpiNodes) + kornia 0.8.2 installed. **Stock stage-gap ~14s COLD with NO hang anywhere** (log: stage-1 done 07:40 → stage-2 start 07:54); full COLD gen **59.79s**. Compare OUR app: **39s gap WARM**, 1:23 full warm, ~5min cold. **Stock COLD (59s / 14s-gap) beats OUR WARM (1:23 / 39s-gap).** Same card, same torch, same aimdo, same workflow → the ~3× is 100% on OUR side.
> **STOCK vs OUR launch cmd (the delta, from the live diff):**
> - stock: `main.py --listen 0.0.0.0 --port 8188 --enable-cors-header` (NOTHING else)
> - ours:  `main.py --listen 127.0.0.1 --port 8188 **--preview-method taesd** --input-directory … --output-directory … --extra-model-paths-config …`
> **Suspects (ranked), all fixable with NO image rebuild (start.sh/wrapper env):**
> 1. ~~**`--preview-method taesd`**~~ **CLEARED LIVE 2026-07-05.** Relaunched the SAME stock ComfyUI WITH `--preview-method taesd` added (only change) + reran the gen: WARM **32s full, 8-10s gap** — still fast, taesd did NOT slow it. So the flag is innocent. Crossed off.
> 2. **5 extra baked node packs** we ship that stock lacks (Impact-Pack, Impact-Subpack, GGUF, UltimateSDUpscale, Frame-Interpolation) — import hooks / model-management patches taxing every gen. Stock had only LTXVideo/KJNodes/MpiNodes.
> 3. ComfyUI 0.27 (ours) vs 0.26.2 (stock) — minor; unlikely but not excluded.
> 4. RAM: stock 386GB vs our 55GB test — a confound, but 14s-COLD vs 39s-WARM is too large a gap for RAM caching alone.
> **NEXT: test suspect #1 first (cheapest) — `--preview-method` on the Pod via CUBRIC_VRAM_MODE-style env/start.sh edit, no rebuild. Then #2 (boot without the 5 unused packs).**
>
> **★★★ NODE PACKS + FLAGS ALL CLEARED (live-bisected 2026-07-05 on the stock Pod). THE CULPRIT IS THE WRAPPER / REQUEST PATH.** Systematically added OUR differences onto the fast stock ComfyUI, one at a time, remeasuring each:
> | added onto stock ComfyUI | warm full | gap | verdict |
> |---|---|---|---|
> | (nothing — plain stock) | ~32-35s | ~8-14s | baseline fast |
> | + `--preview-method taesd` | 32s | 8-10s | flag CLEARED |
> | + `--input-directory`/`--output-directory` | ~35s | ~9s | flags CLEARED |
> | + ComfyUI-GGUF node pack (loaded+patching) | 35.95s | ~9s | GGUF CLEARED |
> | + Impact-Pack present (+ other 4 packs) | ~38s | 8-11s | node packs CLEARED |
> **Every launch flag + every extra node pack we bake = INNOCENT. Stock-with-all-our-additions still runs ~35s / ~9s-gap — vs OUR APP's 1:23 / 39s-gap on the identical card+torch+aimdo+workflow.** The ONE remaining difference not on the stock manual path: **our app drives the gen THROUGH THE CUBRIC WRAPPER** (POST /wrapper/prompt on 8889 → proxy to internal ComfyUI 8188), whereas the stock test queued the prompt DIRECTLY in ComfyUI's web UI. **→ The ~2.5× tax is in the wrapper or the exact prompt JSON our app POSTs.** Investigate OFFLINE (free, no Pod): (a) diff the actual prompt JSON our app sends vs the LTX_t2v.json template — extra nodes / different widget values / LoRA-stack wiring; (b) read the wrapper's prompt-submit path (`wrapper.py`) for added latency, a synchronous stall, a polling loop, or an SSE/queue difference; (c) how the app polls for completion vs ComfyUI's native queue. NOTE minor thread: stock log showed `TAESD previews enabled, but could not find models/vae_approx/None` — taesd preview SILENTLY SKIPS on the Pod (no approx model); in our app the approx model may be present so the preview decode ACTUALLY RUNS every step — re-examine whether taesd is truly free once vae_approx is available (the Pod test never had it, so taesd was a no-op there).
>
> **★★★★ OFFLINE WRAPPER AUDIT (2026-07-05, 3-agent sweep — prompt-JSON + wrapper.py + preview pipeline). PROMPT JSON + PREVIEWS CLEARED; WRAPPER SUSPECTS RANKED; ONE NEW CONFOUND; THE DECISIVE TEST IS MPI-192.**
> - **Prompt JSON CLEARED.** The app POSTs API-format `{prompt, client_id}` (comfyController.js:1189-1193) — the template verbatim plus field-level injections only (prompt/seed/duration/size/LoRA slots, `\`→`/` path normalization). No extra nodes, no `extra_data`, no compute-relevant graph delta vs `LTX_t2v.json`. Not the tax.
> - **Preview path CLEARED (and the taesd thread CLOSED).** `LTXAV.taesd_decoder_name = None` in core `latent_formats.py` → `--preview-method taesd` is a NO-OP for LTX on EVERY pod, ours included; the baked `vae_approx/` models (MPI-98) are irrelevant to LTXAV. LTX previews come solely from in-graph node 366 (`LTX2SamplingPreviewOverride`, KJNodes) = cheap latent2rgb matmul (<1ms/step), wall-clock-limited at 8fps, and it fires on BOTH paths (web-UI queue registers a client_id too). Not the tax.
> - **WRAPPER SUSPECTS (ranked, all app-path-only, none testable by the stock bisect):**
>   1. **`/wrapper/stats` runs `nvidia-smi` as a BLOCKING `subprocess.run` INSIDE the asyncio event loop** (wrapper.py:639-643, no `asyncio.to_thread`) — the app polls it every ~2s during gen; each call blocks the loop 150-400ms on a loaded GPU AND issues NVML queries that contend with the driver exactly while it services the 40GB UVM fault storm. Stock test: nothing touched NVML mid-gen.
>   2. **ComfyUI stdout/stderr is PIPED into the wrapper** (`asyncio.subprocess.PIPE`, wrapper.py:256-259) and drained by a per-chunk regex+SSE loop in the SAME event loop (wrapper.py:445-475, double-decode at 468-469). A blocked loop → pipe backpressure → ComfyUI's own `print`/tqdm can stall the execution thread. Stock: stdout → docker logs, unbounded.
>   3. WS pump forwards every binary preview frame over the WAN (wrapper.py:1716-1722); `_build_loader_index` blocks WS (re)connects up to 5s on `/object_info` (wrapper.py:509-519, 1709). Lower risk.
>   4. Remote-only `/history` poll every 5s through the proxy (comfyController.js:841-849). Cheap, lowest risk.
> - **⚠️ NEW CONFOUND (user recall 2026-07-05): the FIRST stock-image test ran the GGUF **Q8** transformer (~23GB), NOT bf16.** Q8 FITS a 32GB 5090 → aimdo has no reason to evict between stages → near-zero gap; bf16 40GB NEVER fits → forced evict+re-fault. The doc's "identical workflow" claim was never verified on the WEIGHTS axis — the stock-fast-vs-app-slow numbers may compare Q8-that-fits vs bf16-that-doesn't. Does NOT clear the wrapper; DOES mean any verdict test must hold weights constant.
> - **THE DECISIVE TEST (= MPI-192, one pod, one variable):** set `CUBRIC_COMFY_LISTEN=0.0.0.0` (env already exists, wrapper.py:176) + add port 8188 to the pod-create payload (routes/remotePodLifecycle.js) → on OUR image with OUR weights and OUR workflow, queue `LTX_t2v.json` once DIRECT in ComfyUI's web UI and once through the app. Direct fast + app slow = wrapper GUILTY (then bisect suspects 1-3); both slow = image/launch/weights, wrapper acquitted. Kills the Q8 confound simultaneously.
> - **Cheap orthogonal A/B (no code):** run one gen with the app's 2s stats poll disabled — if the gap collapses, suspect #1 confirmed alone. (Watchdog-safe: `generation_active` skips the idle check during gen.)
> - **Fix deployment is CHEAP:** wrapper.py is R2 runtime-fetched at boot (MPI-156, `publish-runtime.sh` → `cubric-pod-runtime` bucket) — wrapper fixes (`asyncio.to_thread` the nvidia-smi, single decode, stdout drain off-loop) go live with NO image rebuild.
> - **★★★★★ WRAPPER ACQUITTED — LIVE A/B 2026-07-05 (5090 32GB, 190GB-RAM host, our v0.12.0 image, bf16, MPI-192 direct-8188 door):** browser-direct gen (NO wrapper, NO app) = **81s full warm / ~35s stage-gap** vs through-app ~83s / ~39s — IDENTICAL. The ~2.5x tax is INSIDE the pod (image/launch/nodes), NOT the request path. The handoff's "culprit is the wrapper" conclusion is DEAD. (Door mechanics that made this possible: `.expose-comfy` marker file OR `CUBRIC_EXPOSE_COMFY=1` env app-side; start.sh `CUBRIC_COMFY_LISTEN` default-only fix; wrapper adds `--enable-cors-header` on non-loopback bind because ComfyUI's origin-only middleware 403s `Sec-Fetch-Site: cross-site` nav — that's WHY stock runpod/comfyui passes that flag.)
> - **🔥 NEW PRIME SUSPECT (found the second the door opened): DUPLICATE + STALE VOLUME NODE PACKS.** ComfyUI's own error dialog: "Duplicate VHS install detected". Import-times log shows FOUR packs loaded TWICE — VHS, KJNodes, MpiNodes, LTXVideo baked in `/opt/ComfyUI/custom_nodes` AND old copies on `/workspace/comfyui/custom_nodes` (the volume, pulled in by the `custom_nodes` line start.sh writes into extra_model_paths.yaml). Volume copies import LAST → OVERRIDE the image-pinned versions → the pod runs STALE LTX-critical packs. The stock bisect installed FRESH packs (9s gap) — this axis was invisible to it. Test running: mv the 4 dup volume dirs aside (GGUF stays — KJNodes hard import-dep) + `/wrapper/restart-comfy` + re-gen direct.
> - **✅ CHANGE SET SHIPPED OFFLINE 2026-07-05 (live test = USER):** (1) wrapper 0.2.24: `/wrapper/stats` readers moved off the event loop (`asyncio.to_thread` both nvidia-smi + cgroup), stdout double-decode fixed — deploy via `publish-runtime.sh stable`, no rebuild; (2) app `CUBRIC_EXPOSE_COMFY=1` env (remotePodLifecycle.js, MPI-192) opt-in exposes 8188 + sets `CUBRIC_COMFY_LISTEN=0.0.0.0` on create — DEBUG only, ComfyUI has no auth; (3) app `CUBRIC_NO_POD_STATS=1` env kills the 2s stats poll (route returns `stats_poll_disabled`) — the suspect-#1 isolation probe. Test order: (a) gen through app with NEW wrapper, stats ON — if already fast, the loop-block was it; (b) same pod, queue `LTX_t2v.json` direct in the 8188 web UI — direct-fast+app-slow = remaining wrapper tax, both-fast = DONE, both-slow = image/launch (weights now bf16 on BOTH paths → Q8 confound dead).
>
> **Node 58 is CLEARED as the cause — the ~11s RunPod number was measured running OUR EXACT `LTX_t2v.json` (node 58 included).** Same workflow, same card class, same torch-family, same aimdo/ComfyUI — yet RunPod's stock image = 11s and OUR image = 43s on a 5090. **→ The gap is in OUR IMAGE BUILD or LAUNCH, not the workflow, not node 58, not torch-minor.** VRAM helps (24GB→32GB halved 86→43s) but does NOT close it (43 ≠ 11 on the same 32GB). **NEXT (the live experiment that matters): redeploy clean v0.12.0 (torch 2.10) to drop the 2.12 regression, then DIFF our Pod's ComfyUI launch + image against RunPod's stock `comfyui-cuda13.0` — some build/launch difference costs ~4× on the same hardware.** Our launch (`wrapper.py:_build_cmd`): `main.py --listen 127.0.0.1 --port 8188 --preview-method taesd [--vram (empty)] --input-directory … --output-directory … --extra-model-paths-config … [--use-sage-attention if USE_SAGE]`. Suspects: `--preview-method taesd` (stock may not preview), extra-model-paths enumeration, a dep/python delta, or a wrapper-launch difference. Also OPEN (user instinct): warm-run RAM caching was NEVER isolated — the old investigation cleared RAM on COLD faults only (62GB L4). 55GB may under-cache the 40GB on warm; a higher-RAM 5090 on v0.12.0 is a clean test.
>
...
> 1. Stage-1 (preview) = one `/prompt` POST (`js/services/comfyController.js:1189`). It runs, saves latents to disk (`Output_Video_Latent` #48 / `Output_Audio_Latent` #54), completes → ComfyUI goes idle.
> 2. In the idle gap, ComfyUI's `soft_empty_cache()` (default `--normalvram`) lets aimdo release the faulted-in 40GB back to the pool. Model evicted.
> 3. User clicks Continue → a NEW, fully-independent `/prompt` POST fires (`isStage2:true`, `MpiGalleryBlock.js` ~548 → `generationService.enqueueGeneration` → same `comfyController.js:1189`). Stage-2 cold-faults the whole 40GB back in. That re-fault IS the 60-93s.
> **aimdo evicts BETWEEN prompts, never between nodes WITHIN one prompt.** (Upstream: comfy-aimdo is a SEPARATE Comfy-Org package — `https://github.com/Comfy-Org/comfy-aimdo/` — not built into core. No `keep-resident`/`no-evict`/`pin` knob and no `AIMDO_*` env in 0.4.10; the only ComfyUI-flag lever is `--highvram`. Upstream threads propose "skip soft_empty_cache for same-model consecutive prompts" — NOT shipped.)
> **⚠️ `--highvram` may NOT be enough even where the 40GB FITS.** Adversarially-verified against ComfyUI issues [#13139](https://github.com/Comfy-Org/ComfyUI/issues/13139) ("Models always unloaded when using dynamic VRAM", open), [desktop #1741](https://github.com/Comfy-Org/desktop/issues/1741) (VRAM 10-11GB→936MB after gen), [#14475](https://github.com/Comfy-Org/ComfyUI/issues/14475): under DynamicVRAM, aimdo's VBAR allocator RELEASES physical VRAM pages between prompts **even in HIGH_VRAM mode** — `--highvram` skips ComfyUI's `soft_empty_cache()` but does NOT override aimdo's lower-level VBAR page release. The upstream "guaranteed resident" combo is `--highvram --disable-dynamic-vram` — but disabling aimdo OOMs us (proven twice, LTX's full 3-stage resident set > RAM). So Path A is a **gamble even on a big card**: it may not hold the pages, and we can't pair it with the disable that would. This makes Path B the only RELIABLE fix.
>
> **The graph is ALREADY single-prompt-capable — the blocker is the two-POST dispatch, not the JSON.**
>
> > **Mechanism restated 2026-08-07 (MPI-452/466/473) — the CONCLUSION still holds, the node names below do not.** There is no `_stage2` file and no `Input_Is_Continue` node any more: ONE graph carries both stages and the gate is the `MpiStageLatents` widget pair `Input_Video_Latent.is_continue` / `.is_preview`, written per run by `_buildParams`. Read the paragraph below for the reasoning, not for the titles. What is unchanged: it is still **two `/prompt` POSTs**, the flag still means "load the latent from DISK vs live", and a true single-prompt run still needs the stage-1 sampler output wired straight into stage-2 (kill the Save→Load round-trip) — re-wiring, not a flag flip.
>
> `Input_Is_Continue` (node 71, `MpiBoolean`) already exists in BOTH `LTX_*.json` and `LTX_*_stage2.json`; stage-2 = stage-1 with that one boolean flipped `true` (`generate_ltx.py:_derive_stage2`, lines 136-140). The `Stage1_Bypass` sampler (node 70) stays in both files; `MpiIfElse` gates route loaded-latent (continue) vs live-latent (preview). So the two files are ONE graph with a flag — but the flag was designed for the two-prompt model (load latent from DISK vs live), so a true single-prompt run needs the stage-1 sampler output to feed stage-2 DIRECTLY (kill the Save→Load round-trip) = graph re-wiring, not just flipping 71.
>
> **THE FIX PATHS (independent, cheapest first):**
> | Path | What | Cost | Works on 24GB (shipped card)? |
> |---|---|---|---|
> | ~~**A2 — `--cache-ram` env**~~ **DEAD** | The child-agent lead (pin weights in RAM → cheap re-fault) is REJECTED. (1) User recall: `--cache-ram` / high RAM caching was already tried and **OOM'd** — it grows the resident RAM footprint until the container dies (same class as the aimdo-disable OOMs). (2) It is the ComfyUI DEFAULT already, so passing it changes nothing. (3) Our own `read_bytes=0` measurement proves the fault is RAM→process not disk, so the PR-#13802 "eliminates disk reloads" claim doesn't apply to our gap. Do NOT test `--cache-ram` on the Pod — it OOMs and can't help. | — | ❌ OOM |
> | **A — `--highvram` env** | keep the transformer resident across the inter-prompt gap. Set `CUBRIC_VRAM_MODE=--highvram`. NO rebuild. | trivial, env-only | ❌ **NO** — 40GB > 24GB VRAM, OOMs (proven twice). 48GB+ ONLY, and even there a gamble (VBAR releases pages in HIGH_VRAM — see caveat above). |
> | **B — single-prompt merge** | one `/prompt` runs BOTH stages; latent stays in VRAM; no idle gap → aimdo can't evict. Re-wire the graph so stage-1 sampler output feeds stage-2 directly (no disk round-trip); app submits ONE prompt. Confirmed by research: within one prompt aimdo protects active VBARs (`free_for_dynamic`) from eviction. | graph re-wiring in `generate_ltx.py` + templates + app submit-path change | ✅ **YES** — eviction becomes impossible on ANY card. The only GUARANTEED fix for the 24GB deployment. |
>
> **PATH B IMPLEMENTATION LEVER — the `prioritize()` aimdo API (a new `MpiNodes` node).** comfy-aimdo exposes `ModelVBAR.prioritize()` (`comfy_aimdo/model_vbar.py`; README verbatim: *"resets the offload watermark of that model to no offloading, giving its weights priority over any other currently loaded models"*). It is a **Python API only — NO off-the-shelf node exists** in ComfyUI core / KJNodes / any pack (verified). But we already ship the `ComfyUI-MpiNodes` pack (home of `MpiClearVram`) — so we can add a complementary **`MpiPrioritize`** node the same way: during `execute()`, fetch the LTX transformer's aimdo VBAR handle and call `.prioritize()`. Drop it at the stage-1→stage-2 boundary to force the 40GB top-priority (no-offload) across the subgraph transition. **⚠️ This bites ONLY within a single prompt** — `prioritize()` protects a VBAR *while it exists*; across the two SEPARATE prompts we ship today, the VBAR is torn down in the idle gap and the next prompt's prioritize is too late. **So `MpiPrioritize` is not a standalone fix — it's the residency mechanism INSIDE Path B's merged single prompt** (keep the transformer top-priority across the in-graph stage boundary that Path B creates). Path A2/A operate at the flag level and don't need it; Path B does. No OOM risk (priority hint, not hard-resident) → safe on 24GB.
>
> **⚠️ PATH B COLLIDES WITH A SHIPPED PRODUCT FEATURE.** The preview→pick-best-of-several→pay-continue-only-on-the-keeper split is a deliberate iterate-cheap gate (user's 2026-06-28 call — see the multi-stage banner below). A single merged prompt runs stage-2 UNCONDITIONALLY → no pick step. So Path B is not a pure perf win; it trades the cheap-preview product gate for the speed. This is a PRODUCT decision, not an engineering one — do NOT ship Path B without the user's explicit call on the feature tradeoff. (Possible reconciliation, unvetted: keep the two-prompt preview for iteration, and add a separate "commit" mode that merges — but that's new scope.)
>
> **Also being tested first (cheaper, orthogonal): MPI-191 lever (a) = torch 2.10→2.12+cu130** (match local's <30s gap on the same arch). If that alone drops the Pod gap near local's, Path B may be unnecessary. Test order: torch bump (built, user live-test pending) → then, only if still slow, this Path A/B decision.
>
> **NOT the same as upstream ComfyUI issue [#14345](https://github.com/Comfy-Org/ComfyUI/issues/14345) — do NOT conflate them.** #14345 = each LTX *sampler step* runs 2-5× slower under aimdo 0.4.x (within-step weight-streaming from slow disk; OPEN, maintainer rattus128 can't repro; recommends `--cache-classic` over `--disable-dynamic-vram`). MPI-191 = the ~60s dead gap BETWEEN stage-1 and stage-2 (inter-prompt eviction). Different symptom, different mechanism. Our re-fault gap is NOT tracked upstream — Fix #2 is Cubric-specific, no upstream patch coming. (Corroborating cu130: a #14345 reporter fixed *their* slowdown by moving to torch **2.10.0+cu130** — independent echo of MPI-187 that the `+cu130` build is the lever, though it doesn't validate our 2.12 bump specifically.)
>
> ---
