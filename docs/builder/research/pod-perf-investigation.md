...
> |---|---|---|---|
> | **A2 — `--cache-ram` env (TEST FIRST — free)** | force evicted weights into PINNED host RAM between prompts → re-fault = PCIe transfer (~1-5s for 40GB) not 60-90s. Set `CUBRIC_VRAM_MODE=--cache-ram` (Pod env → `start.sh:159`), `POST /wrapper/restart-comfy`. NO rebuild, NO OOM risk. Upstream claim: ComfyUI [PR #13802](https://github.com/Comfy-Org/ComfyUI/pull/13802) "gen-to-gen reloads eliminated" for LTX-2.3. | trivial, env-only | ✅ maybe — safe to try on any card. ⚠️ **SKEPTIC:** our own measurement already showed `read_bytes=0` (fault is RAM→process, not disk) AND `--cache-ram` is reportedly the DEFAULT — so "weights paged to disk" may be wrong and this may do nothing. But it's free → test before touching the graph. |
