...
| [krea2-int8-quant.md](krea2-int8-quant.md) | **Krea2 transformer quant variants — CANDIDATES, not adopted (2026-07-10).** Full matrix verified live on the HF API: Turbo has 5 (`bf16`/`fp8_scaled`/`int8_convrot`/`mxfp8`/`nvfp4`), **Raw has only 3 — no mxfp8, no nvfp4**. All three quants are **native in ComfyUI 0.27** (`comfy/quant_ops.py` `QUANT_ALGOS`) — no fork, so the GGUF/MPI-190 disqualifier does not apply. `int8_convrot` = **1.92× faster** than `fp8_scaled` on a 3090 (7.70 vs 14.82 s/img @1024²/8 steps) via Ampere INT8 IMMA. **Does NOT contradict** the "INT8 → skip" line in `quant-and-coldstart-investigation.md` — that was LTX on *Blackwell* (native fp8 ⇒ no int8 gain); this is *Ampere* (no native fp8). Same fact, opposite conclusion, different silicon. Ships as a `variants.arch` axis (LTX proves the machinery; **no `workflowSuffix`** — only the `UNETLoader` weight changes). 7 gates: 2 cleared, 5 open — incl. **reproduce on our Ada 4060 Ti** (bench is Ampere; do NOT assume it transfers) + a seeded face A/B. `nvfp4` (7.15GB) is the only variant that fits 8GB resident, but is Blackwell-only ⇒ likely low value. Read before proposing OR dismissing Krea2 quant work. |
