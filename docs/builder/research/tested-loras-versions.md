...
| Transition | **valiantcat v1.0** | `ltx2.3-transition` | standard | tested earlier (stage-2-only=no effect; stage-1=effect) | **DEFER** — FL-only (substantial env/character change); needs effect-system decision; switchable dep, never merged | effect-system brainstorm |
