...
- **`generationSettings`** — Reuse Prompt snapshot for newly generated items: `{ operation, modelId, injectionParams, mediaItems, previewOnly, controlState? }`. It is convenience metadata for prompt/model/settings replay; materialized I2V snapshots in `Media/.preview-assets/<uuid>/` remain the preferred source for frame media when present.
