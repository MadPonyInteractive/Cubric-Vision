...
a `C:\...\Downloads\...png` image path, style 10 selected, `Input_Is_Edit: true`. Every input
...
`Input_Image`/`Input_Mask` → `string` `''` (self-gate),
`Input_Style_Selector` → `selector` `0`, mode flags
