...
| [workflow-authoring-entry.md](workflow-authoring-entry.md) | raw→API sync via `sync-raw-workflows.mjs`; `validate-injection-rules.mjs` gate; `raw/` writable but LiteGraph-only; staged output | model: converter-staleness trap + `_template` routing (inline in add-model/01) |
