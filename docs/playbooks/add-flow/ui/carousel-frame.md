...
- Step dots indicate position and total length — a flow that reveals its length one arrow at
  a time feels endless. They also **navigate** — see § The step ticker NAVIGATES.
...
- **Inputs survive leaving and coming back, without a run.** `state.s_flowInputs[flow.id]` is
  ...
  so a mid-run edit cannot corrupt what Reuse restores (`03-storage-and-reuse.md`). Do not fix
  one into the other.
...
the run for persisted. `submitFlowGeneration` sets `deferCommit` and sends NO
`placeholderGroup` (nothing is pending in the gallery when it may never land there).
`MpiBaseFlow` holds them in `_pendingGroups`; `_apply` persists them with the SAME
...
is the only difference between a flow run and a normal gallery run.

Decided:

- **Pending results live in flow state and do not survive closing the flow.** No project-level
...
Live previews already have their plumbing: `submitFlowGeneration` returns `tempId`, which
`MpiBaseFlow` uses to match `preview:frame` events (MPI-271). What changes is where the
