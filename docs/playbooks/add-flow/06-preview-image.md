...
| An ODD height on an ffmpeg `color` source | It is silently rounded DOWN to even (yuv420p needs even dimensions): `color=c=…:s=1280x175` yields **1280x174**. Size an opaque reveal cover to match an odd-height overlay and the cover is one row short, so the un-revealed content leaks along the bottom edge — 131 pink pixels on Song's first frame, from a filtergraph that exits 0. Nothing warns, and the leak is invisible frame by frame; it only shows as a flicker at the loop. Keep every overlay dimension and every overlay `y` EVEN, and `ffprobe` the cover if in doubt |
| `fade=t=out:c=<the token hex>` to close a loop | The synthetic tail lands BRIGHTER than a PNG plate carrying the identical hex: `fade`'s `c` is written straight into the output values, while a plate goes through the RGB → limited-range-YUV scaling first. Song measured first frame (24.8,15.2,19.7) against last (28.0,18.0,22.0) — a uniform 3-level pop every 7 seconds, and CRF 24 → 16 moved it by 0.001, which is what ruled quantisation out. Fade the REAL blank plate back in over the top instead (`geq` alpha ramp + `overlay`): both ends are then the same pixels through the same path. Seam went 2.78 → 0.22 /255 |
| Judging a hero from a contact sheet | Everything reads at 620 px. Render at **446 px** — the real hero width — before believing it |
| Screenshotting an element BIGGER than the viewport | `playwright-cli screenshot '#el'` returns the element at its exact CSS size — including the region that was never painted, as **pure black**, with no error and the right dimensions. A 1280-wide hero in a 1000-wide viewport came back with a 280 px black right edge; a 1120-tall tile in a 900-tall viewport came back with its **top 210 px black**, which ate the tile's kicker and shipped as far as the live Flow Library before it was spotted. `resize` the viewport LARGER than the element before every shot, and assert a known-ground pixel at each edge afterwards (`sharp().extract().raw()`, compared against the `--surface-viewer` value the browser actually paints — `rgb(29,18,23)`). "Right dimensions, looks fine at a glance" proves nothing: the black band lands exactly where a top or side margin belongs (MPI-694) |
