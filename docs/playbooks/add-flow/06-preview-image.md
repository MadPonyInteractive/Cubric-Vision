...
| The same input can become **several different things** | ONE seam wipe, then hold the LAYOUT while the LOOK changes. **Scribble** (shipped) — the drawing wipes to an anime render, which crossfades to a photoreal one and back; the composition never moves, so the only thing the eye reads is the style. The loop point IS the tile frame (seam parked at 45%), which is also what stops the `poster` cutting to a different picture when the clip starts. Cheaper than two subjects and it names the flow's actual pitch: the drawing gives the shapes, the words give the look |
