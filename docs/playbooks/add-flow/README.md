...
| File | Covers |
|---|---|
| [01-descriptor-and-ops.md](01-descriptor-and-ops.md) | The `FlowDef` in `flowsRegistry.js`; the op in **4 files**; no-model vs multi-model flows; the uiComponent (optional) |
...
2. **Inputs.** Flows are input-agnostic: a prompt, image(s), video(s), audio, a gizmo, or
   **nothing** (just Run). Declared in `inputSchema` — media slots in `inputSchema.media`,
   other controls via the flow's `uiComponent`. Media is NEVER a hard requirement in v1, but
...
- [ ] uiComponent (controls only)? Register CSS in `preloadStyles.js`, props in `types.js`, map NAME→blueprint in the shell `flow:open` handler. **Omit for a media-only flow** (BaseFlow renders slots) — [01](01-descriptor-and-ops.md), [04](04-overlay-and-shell.md)
