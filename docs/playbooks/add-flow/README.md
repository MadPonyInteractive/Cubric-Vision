...
- [ ] Nothing to do for a VIDEO result's player: a single video result mounts MpiVideoViewer + MpiVideoControlBar automatically, with compare (when declared) as a toggle on top — [04](04-overlay-and-shell.md) § every video result gets the real player
