...
- **Kanban cards MUST track real state — MOVE them:** `todo → doing` BEFORE editing files, `doing → done` when the work ships. A move = update BOTH `board.json` columns AND `tasks/<id>/task.json` (`column` + `maturity` + `updated_at`) + a `task.moved` event in both event logs. Board is JSON — read `<mpi-lib>/task-board-ops/mutate.md` + `.claude/rules/kanban.md`, NOT the legacy `kanban-ops/` Markdown doc. **`<mpi-lib>` = `C:\Users\Fabio\.agents\skills\mpi-lib\`** — use THAT path, not the `~/.claude/skills/` twin: the twin is a SYMLINK `find` will not traverse, so a lookup there returns 0 hits and reads as "the file does not exist" when it does (full trap + the validator's real home in `scripts/` → `.claude/rules/kanban.md`). **Creating a NEW card also needs `<mpi-lib>/task-board-ops/read.md`** (`nextTaskId`): the id comes from `board.json`'s `next_id`, which you then increment — deriving it from the largest existing card id instead leaves `next_id` stale and the validator rejects the WHOLE board, not just your card. **A `todo → doing` move MUST also write `tasks/<id>/files.json`** — `{"schema":"mpi-kanban/files/v1","files":["path",…]}`, the paths that card owns. You are the only one who can: ownership may never be inferred from card text, title or a diff, so nobody can backfill it for you later. Without it the card is not dispatchable by `mpi-execute-parallel` and peers cannot see what you hold. Measured 2026-08-08: 0 of 83 cards declared ownership (the two `files.json` that existed were both empty, one in the wrong shape), which is why parallel work gets hand-rolled with no claims.
- **CLAIM FILES BEFORE EDITING — this tree is shared with LIVE peer agents.** Read `.agents/mpi-kanban/state/index.json` → `active_file_claims` FIRST. A fresh `claimed` record owned by another session means **do not edit that file**: wait, split ownership, send a `mpi-message`, or ask. Then write your OWN claim for what you are about to edit, and release it at close — `<mpi-lib>/coordination-ops/lifecycle.md` (§ Claim Files, § Complete Or Release File Claim); contract + worked shape in `.claude/rules/kanban.md` § File claims. **This binds EVERY agent on EVERY edit, skill-invoked or not** — the claim step lives INSIDE the `mpi-*` skills, so an agent doing ordinary work never loads it and silently overwrites a peer. Measured 2026-08-08: `active_file_claims` was `[]` in every commit from 2026-06-16 onward while 3-5 sessions ran concurrently. That gap IS the clashes.
- **NEVER `git checkout --` / `git restore` / `git stash` / `git reset --hard` / `git clean` on this tree.** They restore from HEAD and take every peer's uncommitted work with them — silently, exit 0, `git status` clean afterwards, which reads as a successful revert. **Blast radius is the PATHSPEC, not your file:** a broad pathspec has wiped a peer's board move, their `events.jsonl` line, and their code AND doc edits in one command. Undo a probe by **re-applying its inverse edit with the same tool that made it**; take a baseline with `git show HEAD:path > /tmp/base.js`, never by stashing; `cp` a backup before a mutation test. After ANY revert, grep for a distinctive token of your OWN work — a clean `git status` cannot tell a wipe from a success. Full case: `.claude/rules/git.md` § MPI-365.
...
3. **Declare the sub-agent's OWNERSHIP and make it claim.** Name the exact files/modules it may edit, paste the two coordination bullets from the Critical Rules Snapshot (claim-before-editing + the destructive-git ban), and tell it: read `active_file_claims` in `.agents/mpi-kanban/state/index.json` at startup, claim ONLY its owned paths before its first edit, release them in its final report, and — if it needs a file outside its ownership — file one `mpi-message` and STOP that line of work rather than editing or negotiating. A sub-agent dispatched without ownership is the single biggest source of peer clashes in this repo.
