...
| Debugging / crashes / python engine issues | Last 50–100 lines of `logs/app.log` (`Read` with offset — NEVER the whole file). **Never read `logs/app.log.1`** (rotated backup) unless the user explicitly asks — it is the evicted overflow and reading it burns context for almost no signal. ComfyUI stdout noise is console-only by design (MPI-315), so app.log is the app's story, not the engine's; for engine detail ask the user for the terminal output |
