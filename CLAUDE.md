...
| Adjust (grow / shrink / edge band off one `_morph`, the measured radius table, **Fill Holes** and its opt-in composite flag) | `docs/masking-adjust.md` |
...
| Running the node test suite | `docs/DEVELOPMENT.md` § Tests — `node --test "tests/*.test.cjs"` (quoted glob). The directory form `node --test tests/` DIES on Node v24 (`Cannot find module '...\tests'`) |
