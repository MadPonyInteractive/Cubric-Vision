...
- **EVERY UI ELEMENT IS A COMPONENT** — never a bare `<input>`/`<select>`/`<textarea>`/`<button>`. Slider = `MpiProgressBar`, select = `MpiDropdown`, checkbox = `MpiCheckbox`, text/number = `MpiInput`, segmented choice = `MpiRadioGroup`, action = `MpiButton`. Nothing covers the use → **create a new component**. **Flows are no exception** — a Flow's declared `fields` NAME these components, they do not replace them. A consumer block may only SIZE a Primitive, never restate its chrome (`accent-color` in a consumer stylesheet = a native widget got through). `mpi/no-bare-form-control` warns; the law is `.claude/rules/components.md` § Every UI element is a component (MPI-582).
...
| Make a Flow's **graphics** — its 4/5 tile still + its wide autoplaying hero clip | `/mpi-flow-graphics` | `docs/playbooks/add-flow/06-preview-image.md`. Worked examples: Add Foley (waveform draws in sync), Head Swap (before/after wipe), Extend Video (progress rail past the source-end mark) |
...
Agents MAY commit without asking. Shared tree — commit by explicit pathspec (`git commit --only <paths>`), never `git add -A`/`git add .` (full co-owned-file recipe: `.claude/rules/git.md`). **Agents MAY also push without asking** — `push_policy` in `.agents/mpi-kanban/project-profile.md` frontmatter is the source of truth and it is `auto` (Fabio, 2026-08-27; this line used to say the opposite and made close-out stop to ask every time). **A push to master is still refused by `.husky/pre-push` while master's last CI run is RED** — that gate exists because one broken build survived nine pushes on 2026-08-23 (a Flow descriptor named preview art nobody had made; the 404 reddened the desktop suite). `git push --no-verify` when you ARE the fix. It fails OPEN — no `gh`, no auth, no network, no run found and the push proceeds — and never blocks on a `cancelled` or in-flight run, so a block means a real `failure`. Docs-repo push block above still applies.
