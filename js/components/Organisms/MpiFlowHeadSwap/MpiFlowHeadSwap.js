...
/**
 * MpiFlowHeadSwap — controls for the Head Swap flow (MPI-306 Phase 2).
 *
 * CONTROLS ONLY (composition): MpiBaseFlow owns the frame, the two media slots,
 * both box steps, Run, and the result pane. This component renders the ONE knob
 * this flow has — the speed/quality tier — and owns the translation from the
 ...
 * means; the injector wants `box1`/`box2`. Which ROLE feeds which NODE is flow
 ...
 * Crop). Teaching the frame that mapping would make every future flow's roles a
...
export const MpiFlowHeadSwap = ComponentFactory.create({
    name: 'MpiFlowHeadSwap',
    css: ['js/components/Organisms/MpiFlowHeadSwap/MpiFlowHeadSwap.css'],

    template: () => `
        <div class="mpi-flow-head-swap">
            <span class="mpi-flow-head-swap__label">Speed</span>
            <div id="head-swap-tier"></div>
            <span class="mpi-flow-head-swap__cost" id="head-swap-cost"></span>
        </div>`,
    ...
    setup: (el, props) => {
         ...
         * @returns {Object} inputs merged into the Run config by MpiBaseFlow.
         */
        el.getInputs = (ctx = {}) => {
