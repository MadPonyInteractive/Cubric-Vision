 ...
 * @property {string} id          - Stable id, recorded in the acceptance receipt.
...
export const MODEL_LICENCES = {
    ...
    // Section V.2 is why this gate exists at all: we must bind each user to terms at
    // least as protective as Section V and Exhibit A BEFORE giving them access, and
    // tell them the restrictions apply. Section V.5 is why `report` exists.
    'minimax-h3': {
...
/**
 * Has the user already accepted THIS version of THIS model's licence?
 * A `version` bump in the descriptor invalidates every prior receipt, which is how a
 * revised AUP reaches users who installed under the old one.
 * @param {string} modelId
 * @returns {boolean}
 */
export function hasAcceptedLicence(modelId) {
    ...
    const r = Storage.getLicenceReceipts()[modelId];
    return !!r && r.licenceId === licence.id && Number(r.version) >= licence.version;
}
...
export function recordLicenceAcceptance(modelId, at) {
    ...
    all[modelId] = { licenceId: licence.id, version: licence.version, at: at || new Date().toISOString() };
