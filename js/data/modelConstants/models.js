 ...
 * @property {{multiStage?:boolean, audio?:boolean, negativePrompt?:boolean, styleLoras?:boolean, promptEnhance?:boolean, tierSelect?:boolean}} [capabilities] - Drives capability-gated UI on SHARED ops: multiStage shows the previewStage toggle; audio shows the audio media slot; styleLoras shows the style picker + Stylization slider; promptEnhance declares COMFYUI-BACKEND ELIGIBILITY (see below) and no longer shows any UI at all; tierSelect shows the runtime speed/quality tier radio (Qwen-Image-Edit's qwenTier → Input_Tier, MPI-300) for models whose tiers share one weight set instead of shipping as sibling cards. Absent → false. EXCEPTION: negativePrompt defaults to TRUE when absent (a model supports negatives unless it opts out) — set `negativePrompt: false` for distilled cfg-1.0 models (Krea2-Turbo) where the negative prompt has no effect and NAG cannot rescue it. Hides the prompt box's positive/negative toggle; the stored negativePrompt value is still persisted. `promptEnhance` CHANGED MEANING IN MPI-677 AND KEPT ITS VALUE: it used to gate the in-workflow enhance toggle, which is deleted — enhancement is now its own control beside the prompt box, on every model and every workflow, and the four graphs that carry `Input_enhance_prompt` bake it `false` with nothing injecting the key. The flag now declares that this model's graph carries a `.generate()`-capable CLIP (Qwen3-VL, Gemma), i.e. that it is eligible for the LOCAL ComfyUI enhance backend, which reuses the encoder a generation already loads instead of standing a second runtime up beside it. `canEnhanceInGraph()` in `js/services/llmService.js` is the only reader. T5/umT5 models (Chroma, Wan) CRASH on the TextGenerate node, so never set it there — those models still enhance, on DeepInfra or Ollama, they just cannot do it in-graph.
