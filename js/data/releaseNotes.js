...
export const RELEASE_NOTES = {
  '1.3.0': {
    ...
    fixes: [
      ...
      'Linux: setting up the engine on a machine without an NVIDIA card no longer downloads gigabytes of NVIDIA-only components. The setup correctly chose the processor-only build, but the tool that works out what to download ignored that choice and fetched the full NVIDIA graphics stack anyway — several gigabytes of code that machine can never run, on exactly the kind of hardware least able to spare the download or the disk. It now installs the processor-only build. Windows and Mac were never affected.',
    ],
    breakingChanges: [],
    engineNotes: [],
  },
