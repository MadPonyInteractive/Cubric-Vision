...
export const RELEASE_NOTES = {
  '1.3.0': {
    ...
    fixes: [
      ...
      'Linux and Mac: Retry now works when engine setup fails. If setup stopped partway through for any reason — a dropped connection, a power cut, a full disk — every later press of Retry failed instantly, because the Python environment left behind by the first attempt was still there and the setup tool refuses to replace one. There was no way forward from inside the app at all; the only fix was deleting a folder by hand that nobody would think to look for. Setup now replaces that environment cleanly, so Retry does what it says.',
