...
export const RELEASE_NOTES = {
  '1.2.0': {
    ...
    importantChanges: [],
    whatIsNew: [
      'PICK YOUR OPERATION FROM A STRIP — the operation is no longer buried in a dropdown. A strip of chips sits above the prompt box showing every operation your model can do, with the current one lit. An operation you cannot run right now stays visible but dimmed, and hovering it tells you exactly what is missing (needs 1 image, paint a mask first, takes at most 2 images) instead of vanishing from the list. The strip also appears inside the parameters popup, so you can change operation without closing it. Clearing the last image from the prompt box now drops you back to text-to-image automatically, and your chosen operation comes back when you add an image again.',
      'FULL-SCREEN MODEL PICKER — choosing a model opens a full-screen contact sheet of preview tiles instead of a list, matching the Model Library. Holding Tab opens it straight away with no ring to aim at, so the gesture is the same every time regardless of which model you are on. LoRA & Upscale settings open from the model tile itself.',
