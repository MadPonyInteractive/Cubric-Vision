...
export const RELEASE_NOTES = {
  '1.3.0': {
    ...
    importantChanges: [
      ...
      'ALREADY ON WINDOWS? If the app currently does not start for you at all, updating cannot fix it — the updater is a .bat and is blocked by the same rule. Download the full build from the release page and unpack it fresh; your projects live in Documents and are untouched. If you update in place, an old app folder is left behind in your Cubric Vision folder — nothing uses it any more and it is safe to delete, about a gigabyte back. Updating from inside the app is now the recommended way on Windows and works regardless of Smart App Control.',
    ],
    whatIsNew: [
    ...
    fixes: [
      ...
      'Mac and Linux: Retry after an interrupted setup now repairs it properly. The first engine install is several gigabytes, so quitting partway through is easy to do — but coming back and pressing Retry finished in seconds, announced success, and left an engine that could not start, with no way out from inside the app. Retry was deciding what to do from a file that exists from the very first moment of the install, so a half-finished setup looked complete and only the extras were reinstalled. It now checks whether the install actually finished, re-runs the part that did not, and keeps what you already downloaded. Windows was never affected.',
      ...
      'Linux and Mac: Retry now works when engine setup fails. If setup stopped partway through for any reason — a dropped connection, a power cut, a full disk — every later press of Retry failed instantly. Two separate leftovers from the first attempt caused it: the Python environment it had created, and the engine folder it had already downloaded. The setup tool refuses to touch either one if it finds it there, so it gave up before doing anything, every time. There was no way forward from inside the app at all; the only fix was deleting folders by hand that nobody would think to look for. Setup now replaces the Python environment cleanly and picks up the existing engine folder where it left off, so Retry does what it says.',
    ],
    breakingChanges: [],
    engineNotes: [],
  },
