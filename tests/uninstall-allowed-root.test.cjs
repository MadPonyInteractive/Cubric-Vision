...
(async () => {
    ...
    // 1 — the bug itself, against a REAL registry dep rather than a fixture.
    //
    // On master this is `chatterbox-t3`, Text to Speech's 2GB transformer — the weight
    // the user's uninstall could not free. That dep is a FLOW dep and does not exist on
    // this 1.4.x line, which carries no Flows, so the fixture here is `rife47`: the
    // frame-interpolation ckpts, which are the other real engine-anchored weight and the
    // exact case assertion 4 below calls out. Same shape, same bug, dep that exists.
    const weightInNode = DEPS['rife47'];
    assert.ok(weightInNode, 'rife47 missing from DEPS');
    assert.ok(weightInNode.targetPath, 'rife47 must be a targetPath dep for this test to mean anything');
    assert.strictEqual(
    ...
    // master asserts >= 2; this line carries exactly one targetPath dep because the Flow
    // deps that make up the rest are not here. One still exercises the loop below.
    assert.ok(targetPathDeps.length >= 1, 'expected the registry to carry at least one targetPath dep');
