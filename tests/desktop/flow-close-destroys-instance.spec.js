...
/**
 * MPI-345 — closing a Flow must DESTROY its instance, not just hide the overlay.
 *
 * A hidden-but-alive flow keeps every listener its setup registered, including the
 * global `generation.run` hotkey. That is what fired a phantom Head Swap alongside
 * a Qwen edit: one Ctrl+Enter reached BOTH the PromptBox and the closed flow's Run.
 * The handler count on `down:control+enter` is the invariant — it must return to
 * its pre-open value once the flow closes.
 */
// Electron boot (splash → local server → shell) plus the settle wait below runs
// past the 30s default.
test.setTimeout(90000);

test('closing a Flow releases its generation.run hotkey', async ({}, testInfo) => {
  ...
  // `app` here is the ElectronApplication handle from launch.js — the Electron
  // application, not a Flow. Do not rename it.
